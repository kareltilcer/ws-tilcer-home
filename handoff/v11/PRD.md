...
> Status: **v10.2 — LIVE** (the shell's own chrome, deployed 2026-09-05/06) · **v10.1 — LIVE** (chat's second pass, 2026-08-30) · **v10 — LIVE** (Chat, deployed 2026-08-28) · **v9 — LIVE** (Soukromé položky a Úložiště, deployed 2026-08-25) · **v8 · v7 · v6 · v5 — LIVE** · **v11 — ALL THREE PRs BUILT, NOT YET DEPLOYED** (MCP; §V11-1…§V11-11, **§V11-12 is the as-built record and should be read first**, decisions **D275–D323**, three PRs per D323 — PR 1 the host, the token store, the `via` attribution and the `todo`/`events`/`notes` providers; PR 2 the other seven providers, the search fan-out and the resources; PR 3 the two Asistenti screens, the prompts, `backend/mcp-manifest.json` and the load test. ⚠ **The deploy still needs the `/mcp` Coolify path route**, which nothing in the repo can do — D318) · ⚠ **`fin` is not yet retired** — the migration is **verified** (§V6-12 step 4 done) and only the ordered switch-off (step 5) is outstanding. **Eleven modules live. v9 and v11 each add none** — v9 gave Poznámky and Dokumenty a second, private root and added `platform/storage`; **v11 adds no module either: it adds a second FRONT DOOR to all eleven**, an MCP server inside the same binary, reached with a personal access token and publishing 39 tools through a fifth registered catalog. v10 added the eleventh module, **Chat**, and with it a third access axis (*membership*). v8 added **Elektřina**, v7 **Zahrada**, v6 **Finance** (absorbing `fin.tilcer.cz`), v5 **Administrace** + the PWA. Self-hosted login (Mode B), widget dashboard and modular architecture unchanged throughout. Decisions **D1–D323** (§10 for D1–D50; the **v5 section** for D51–D74; **§V5-12** for D75–D80; the **v6 section** for D81–D98 with D99/D100 taken during the build and **§V6-13** the reconciliation; the **v7 section** for D101–D132 with **§V7-12** = D163–D168; the **v8 section** for D133–D162 with **§V8-12** = D169–D175; the **v9 section** for D176–D215 with **§V9-12** the as-built record — including **D214, which the build DECLINED**; the **v10 section** for D216–D262, with **§V10-12/13/14** as built and D263–D270, and **§V10-15** = v10.2, D271–D274, with **§V10-16** and **§V10-17** the two passes that shipped between v10.1 and v10.2 with no record and were written up retroactively on 2026-09-06; the **v11 section** for **D275–D323**, of which **D278, D290, D297 and D323 were settled with Karel after the section was written**, with **§V11-12** the as-built record). ⚠ **D257 is withdrawn and D264 is a collision.** v-deltas in `CHANGELOG.md`. · Last updated: **2026-09-07** · Owner: Karel
> Companion spec: `openapi.yaml` (OpenAPI 3.1 — **0.15.0 = v11, the MCP token management surface**; **0.14.0 is what is DEPLOYED** and is v10.2-era, adding the same-day reminder lead `"0d"`; 0.13.0 was v10.1, 0.12.2 the cursor encoding, 0.12.1 the v10 as-built reconciliation, 0.12.0 the v10 spec, 0.11.0 v9 as built) — this file is a copy of the served `backend/openapi.yaml`, so the v7/v8 drift cannot repeat. ⚠ **The MCP surface itself is NOT in `openapi.yaml` and cannot be** (D320): it is JSON-RPC with a JSON Schema per tool, and its contract is `backend/mcp-manifest.json`, generated from the live registry by `internal/platform/mcp`'s golden test and committed, with `internal/arch/mcp_completeness_test.go` reading it back (§V11-12). · Notes: `notes.md` · Design brief: `HANDOFF-design.md` (v2–**v11** addenda) · Build: `HANDOFF-1…13-*.md` (v9 = `HANDOFF-11-privacy-storage.md`, v10 = `HANDOFF-12-chat.md`, **v11 = `HANDOFF-13-mcp.md`**) · v11 scope brief: `V11-MCP-brief.md` (frozen 2026-09-06)
>
...
⚠ **This list stops at v8 and has not been extended since.** v9, v10, v10.1 and v10.2 all shipped without a bullet here; their sections (§V9-*, §V10-*) are the record. The v11 entry below is written in the list's own form so the habit restarts; **v9 and v10 bullets are still owed** and are a reconstruction job, not a v11 one.

- **v11 (spec 2026-09-06 · NOT BUILT)** — adds **no module and no migration block**. It adds a second **front door** to all eleven: an **MCP server inside the same binary**, `POST /mcp`, mounted OUTSIDE the `/api` group and authenticated by a **personal access token** — the first bearer credential Home has ever accepted for its own data (D284/D287), minted in Nastavení, hashed like a session, expiring on a schedule its owner picks and **never sliding** (D285). Modules publish through a **fifth registered catalog**, `platform/mcp`, an optional `Source` type-asserted at composition exactly as `metrics` and `lists` are and deliberately **not** part of `registry.Module` (D276/D56). **39 tools** under a ceiling of 45 (D293) — seven cross-cutting, thirty-two per module — plus three resource templates and five Czech prompts. Its defining choice is asymmetry: **narrow on verbs, wide on reads.** Delete, publish, purge, upload, membership, admin mutation and season close are **absent from the tool list rather than gated** (D295), and no tool carries `destructiveHint` (D294); but a token sees **everything its owner sees**, private `soukromé` roots and chat bodies included — so the whole risk of the version is in **§V11-4 FR-M11, the seventeen-row leak table**, not in its write list. Chat is **read-only** (D296), for two independent reasons. Two migrations, no new block: `02005_mcp_tokens` and `01003_audit_via` — the second adds `via` + `via_token_id` by `ALTER TABLE ADD COLUMN`, because a fourth `actor_type` would mean rebuilding `audit_events` **and firing `audit_changes`' cascade while reporting success** (D290). Two *reads* become audited for the first time — `chat.read` and `notes.private.read`, the container and never the content (D297). Six defaulted env vars, a semaphore of two per token against the **one-connection pool** (D304), and a new Coolify path route. OpenAPI → **0.15.0** (144 paths → 148, 283 schemas → 289) covering only the token-management half; the MCP contract is `mcp-manifest.json` (D320). Decisions **D275–D323**. Ships as **three PRs** (D323). Resolved brief: `V11-MCP-brief.md`; build guide: `HANDOFF-13-mcp.md`.
...
### Taken for the shell's own chrome (D271–D274, 2026-09-05)

From the `design/v10_2` bundle plus three requests from Karel. None of them touches a module, an endpoint or the schema: this is the frame the eleven modules are drawn inside, and two of the three are corrections to drift rather than new design. ⚠ **D274 is not one of the three.** It was taken after the pass merged, against a disagreement the review found between two files that had both been claiming to say which release this is.
...
- **D271 — The deployed version is visible, and it says nothing else.** `v1.10.2 · a3f9c2e` (version · short commit; the version half read `v10.2` as first built — D274) in the foot of the desktop side nav and at the bottom of the mobile "Více" sheet: mono, muted, no border, no action. ⚠ **It exists so a bug can be reported.** "It broke" cannot be checked against a rebuild; a string copied into the feedback widget names the exact bundle that broke, which is the difference between a report and a rumour. **It is not a changelog and not an update notice** — D26 stands, the service worker updates the app on its own and nobody is asked to act on a version. ⚠ **The two halves have two sources on purpose**: the VERSION is a property of the code and is read out of `frontend/package.json` — a literal in `frontend/src/platform/version.ts` as first built, changed by D274 — bumped with the CHANGELOG; the COMMIT is a property of the build, arriving as a Vite build arg defaulted from Coolify's `SOURCE_COMMIT`. A build given no commit prints the version alone rather than nothing, and **a value that is not a sha is dropped rather than printed** — the failure to expect is an unexpanded `$SOURCE_COMMIT` or a branch name, and a label nobody can trust is worth less than a label with one half missing. ⚠ **Coolify excludes `SOURCE_COMMIT` from the build by default**; *Include Source Commit in Build* is the toggle, and it is stated in the README beside the arg. ⚠ **The toggle is necessary and was not sufficient** — see the outstanding list and the README: the first deploy with it on still shipped a commit-less label, because a default written in the Dockerfile beats the `ARG SOURCE_COMMIT=<sha>` line Coolify splices in and the file said `ARG SOURCE_COMMIT=""`. ⚠ **Which supply path that deploy actually took is NOT established**, and the outstanding list is where it stays open. The arg must be declared **bare** to inherit whatever the platform supplied — right under every path — and `version.test.ts` asserts that it is, and that it still sits above the `VITE_APP_COMMIT` line that reads it. ⚠ **The reason this decision originally gave for leaving it off — that the variable changes every commit and would invalidate Docker's layer cache — was wrong about this Dockerfile**, and is corrected under D274: `RUN npm ci` sits above `COPY . .`, so the arg never re-runs the install whether the toggle is on or off. (A *release* commit does re-run it, because D274 put the version in `package.json` — that cost is D274's, not the toggle's.)
...
- **D274 — The version the app prints is `frontend/package.json`'s, and nothing else is allowed to claim it.** `APP_VERSION` was a string literal in `frontend/src/platform/version.ts` while `package.json` carried Vite's scaffold default `"0.0.0"` — two files answering "which release is this", which had never agreed and could not notice each other. The manifest is what tooling reads; the literal was what a member copies into a bug report. ⚠ **The label prints the package version VERBATIM rather than reconstructing the CHANGELOG's spelling**, so the string in a bug report is greppable in the one file a release actually bumps. ⚠ **It therefore differs from the CHANGELOG's number by a leading `1.`**: npm demands three-part semver and the releases here are numbered `v10`, `v10.1`, `v10.2`, so the RELEASE is carried in `minor.patch` and the major is a constant `1` meaning nothing. `1.10.2` is v10.2; `1.11.0` will be v11. ⚠ **The mapping is asserted, not documented** — `version.test.ts` reads `handoff/v10/CHANGELOG.md` and fails when a release moves the manifest without the changelog or the reverse, which is the same idiom `nav.test.ts` uses for the nav order. It also asserts that the CHANGELOG is **newest-first**, because the regex takes the file's FIRST `## v` heading and that is only the newest release while the order holds — a release recorded below an older one would otherwise leave the mapping reading the wrong entry and saying nothing about it. ⚠ **`package-lock.json` is part of the fact, and NOTHING IN NPM enforces it.** `npm ci` validates the dependency tree and ignores the lock's root `version` outright: a lock at `0.0.0` under a manifest at `1.10.2` installs and exits 0 with no warning, measured both on an empty tree and with a real dependency. The reason the lock has to move is that its root `version` is *derived* from the manifest, so the next `npm install` anybody runs rewrites it — left behind, it surfaces as an unexplained line in a diff about something else. That is a rule with no reader, so `version.test.ts` is the reader: it asserts the lock's root `version` and its `packages[""]` version against the manifest. ⚠ **And the cache argument against `SOURCE_COMMIT` does not survive reading the Dockerfile** (D271 repeated Coolify's reasoning without checking it): the layer under the arg is `RUN npm run build`, while `RUN npm ci` sits ABOVE `COPY . .` keyed only on the two manifest files, which the arg cannot reach — so the toggle never re-runs the dependency install. (⚠ A *release* commit does, because this very decision put the version in `package.json`. That is D274's own cost, and the toggle adds nothing on top of it.) The only builds that pay for the toggle are commits touching nothing under `frontend/`; on any commit that touches something, `COPY . .` has already invalidated the build layer. **Turn the toggle on.**
...
### The version had two sources and they had never agreed (D274, after merge)

`APP_VERSION` shipped as the string literal `'v10.2'` in `frontend/src/platform/version.ts`, and `frontend/package.json` still carried Vite's scaffold `"0.0.0"` from the day the repo was created. Both answer *which release is this*; neither could see the other. ⚠ **The interesting part is that this was invisible from both ends** — the manifest is read by tooling that never renders, the literal is read by a member filing a bug and never by a tool, so nothing in nine releases had cause to compare them. It is the same shape as D272's phone header: not a bug in either file, a fact split across two files with no reader in common.

The label is now `package.json`'s version printed verbatim, which costs a `resolveJsonModule` in `tsconfig.app.json` and buys the guarantee that a release cannot bump one and forget the other. ⚠ **The manifest does NOT reach the bundle**: the import is named (`{ version as packageVersion }`), so Vite tree-shakes it to the single string — checked in `dist/assets/*.js`, where `1.10.2` appears and `oxlint`, `devDependencies`, `tailwindcss`, `playwright`, `test:e2e` and `vite-plugin-pwa` return nothing. ⚠ **And `package-lock.json` had to move with it** — though not for the reason it is tempting to give. `npm ci` does *not* check the lock's root `version` against the manifest: measured both ways, a lock at `0.0.0` under a manifest at `1.10.2` installs and exits 0. The lock has to move because that field is derived from the manifest and the next `npm install` rewrites it regardless, so a stale one turns up later as a line nobody can explain in a diff about something else. `npm install --package-lock-only` touched exactly the two root `version` fields and no dependency in the tree. ⚠ **And because npm will not check it, `version.test.ts` does** — both root fields against the manifest, proven by desyncing the lock to `0.0.0` and watching the assertion fail. The same review added the other missing guard: the CHANGELOG regex takes the file's FIRST `## v` heading, which is the newest release only while the file stays newest-first, so the ordering is asserted too — with `## v10` renamed to `## v11` further down the file, the old assertions all passed and the new one failed.
...
- **The label reads `v1.10.2`, and the artboards' own sample is `v10.1 · a3f9c2e`** (D274) — not `v10.2`, which appears nowhere in the bundle: the label is new in `design/v10_2` and the bundle was synced against a repo still at v10.1, so its number was never this release's either. npm demands three-part semver, so the release is carried in `minor.patch` and the leading `1` is a constant that means nothing; the alternative was reconstructing the changelog's spelling from the manifest, which makes the string in a bug report ungreppable in the file a release bumps. What the artboards specify is the label's *shape*, and the shape is unchanged.
...
- [ ] **The COOLIFY half of the path has still not been seen WORKING.** Under D274 the label was run with `VITE_APP_COMMIT` set to a real sha and rendered `v1.10.2 · b4a3b32` in both places — side nav foot and *Více* sheet — so `import.meta.env` → `shortCommit` → label is observed rather than reasoned about, and a production `npm run build` with the arg set carries the sha into `dist/assets/*.js`. ⚠ **The outer link has now been exercised once, and it failed.** The first deploy with *Include Source Commit in Build* on and the cache cleared still baked an empty `VITE_APP_COMMIT` into `dist/assets/*.js` — the deployed bundle was the right code (`1.10.2` present) with an empty commit. The defect is one sentence: **a default written in the Dockerfile beats the `ARG SOURCE_COMMIT=<sha>` line Coolify splices in** — in-stage it resets the injected value, above the `FROM` it shadows it — and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, asserted by `version.test.ts` along with its position. Reproduced on the real Dockerfile across every supply path — global splice, in-stage splice, `--build-arg` with a value, a key-only `--build-arg`, an explicitly empty one, and nothing at all — reading the value back out of `dist/assets/*.js`; the README carries that measurement as a table, and bare is the only spelling right under all of them. ⚠ **WHY THE DEPLOY FELL THROUGH TO THE SPLICE IS NOT ESTABLISHED, and two readings have already been wrong about it.** The first denied that Coolify treats `SOURCE_COMMIT` differently from a user build variable; the second asserted the difference *as the cause*. What holds up is the Docker half, which is measurable here: a `--build-arg` that carries anything beats every default in the Dockerfile, `=""` included, while the spliced `ARG` line loses to one — so if the flag path had fired, the label would have been correct, and something kept `SOURCE_COMMIT` out of the build's environment. ⚠ **The Coolify half is NOT measurable here and this document does not settle it**; whether an application variable of that name existed was never checked before the fix shipped, and the README's checklist starts there for that reason. **The bare `ARG` is correct under every path either way**, which is why it shipped without waiting for the answer. ⚠ **What is STILL unobserved is the fixed path on the real platform** — everything here is a local reproduction, not a Coolify build. The README carries the checklist and is the one place it lives; the bundle is what settles it. ⚠ **The Coolify-side control names are the README's business, and Coolify's SOURCE is the authority on them — its published docs still show only the older name** — four documents each carrying their own copy of a third party's panel labels is how this note acquired, and then had to shed, a UI generation that never existed. ⚠ **The cache cost of turning it on was overstated** (D274): `npm ci` sits above `COPY . .` where the arg cannot reach it, so only a commit touching nothing under `frontend/` pays anything at all. (`npm ci` *does* re-run on a RELEASE commit, because D274 put the version in `package.json` — that is D274's own cost, and the toggle adds nothing to it.)
...
## V10-16. As built — the same-day reminder lead (built 2026-09-03, OpenAPI 0.13.0 → **0.14.0**)

⚠ **This section and §V10-17 are written retroactively, on 2026-09-06, as v11's first commit.** Both passes shipped between v10.1 and v10.2 with **no PRD and no CHANGELOG entry of any kind**, which is how the served contract came to be at 0.14.0 with nothing anywhere saying why. They are numbered under §V10 because they shipped in the v10 line and before §V10-15's v10.2, not because either belongs to the Chat version; the numbers are appended rather than interleaved so that every existing cross-reference to §V10-12…§V10-15 still resolves. Reconstructed from the merged diff of [#46](https://github.com/kareltilcer/ws-tilcer-home/pull/46), not from a spec — **there was no spec**, which is itself the thing worth recording.

**What shipped.** Okno do budoucnosti offered *Připomenout **předem*** and five lead times, the shortest a whole day. An event a member wants to be reminded about **on** the day — take the bins out, a birthday, an appointment — had to be entered as a reminder for yesterday, or as no reminder at all. The checkbox now reads *Připomenout*, and the lead selector opens with **v den události**: a `reminder_lead` of `0d`.

**The lead arithmetic needed nothing.** `subtractLead` returns `occurrence − lead` and `0d` is the identity, so the reminder enters the Připomínky widget on the event's own morning, reaches that day's 08:00 summary through the existing `events.pripominky_today` selection (D99), and turns overdue the next day exactly as a `1d` lead does one day earlier. The explicit `case "0d"` exists to *say* so, not to change the answer — the default arm is the fallback for an **unrecognised** stored lead, and a typo'd lead must not silently behave like the same-day one.

⚠ **The migration is the part that could have destroyed data, and it is the closest precedent v11's D290 has.** `reminder_lead` is `CHECK`-constrained and SQLite cannot ALTER a CHECK, so admitting `0d` means rebuilding `events`. ⚠ **It is the first table rebuilt in this repo with INCOMING foreign keys**: `event_links` and `event_reminder_completions` both declare `REFERENCES events (id) ON DELETE CASCADE`, and with `foreign_keys=ON` (`platform/db/db.go`) a `DROP TABLE events` performs an implicit DELETE of every row before dropping, **firing both cascades** — every event link and the household's entire completion history, deleted, with the migration reporting success.

`04002_reminder_lead_same_day.sql` therefore **renames the parent rather than dropping it**. A rename with `foreign_keys=ON` rewrites every REFERENCES clause naming the table, so both children then point at `events_old`; each child is rebuilt to point back at the new `events`, and only then is `events_old` dropped, by which time nothing references it. **No row is deleted anywhere in the file** — every table moves by copy, with column lists written out rather than `SELECT *`, and the two `idx_events_*` are created last because the rename carries the old ones along under their own names.

⚠ **The down migration remaps `0d` → `1d` BEFORE it narrows the CHECK**, and the ordering is load-bearing: the narrow CHECK rejects `0d` on the copy step and fails the migration halfway. `1d` reminds a day early — a harmless over-delivery — where NULL would additionally require clearing `reminder_enabled` to satisfy the second CHECK, and the event would quietly stop reminding anybody. These are the household's own events, not a log of a feature being rolled back, so the answer differs from `08003`'s.

**The contract.** `backend/openapi.yaml` → **0.14.0**. All five schemas that name a lead took the new member — `Event`, `EventCreate`, `EventUpdate`, `Occurrence`, `DashboardReminder` — because a lead the creator can choose is one every reader of that event must render. Minor rather than patch: nothing stored changes meaning, but a client validating against a pinned 0.13.x copy would reject an event the household creates with it. ⚠ **This is the bump that had no CHANGELOG entry**, and 0.15.0 would have stacked on the gap.

**Two smaller things worth keeping.** The lead selector reserved `min-h-[44px]`, one chip row — but the original five chips need 362 px against a 335 px container at 375 px, so it has always wrapped to two rows and 72.7 px, shoving everything below it down by 29 px on every tick; the magic number is gone rather than raised, and conditional blocks now go through a `Reserved` component that is always laid out and merely hidden. And `reminderLead.ts` holds each lead once with a `chip` and a `detail` string: `detail` says the direction outright (🔔 1 den předem) because it is read alone, `chip` stays terse because it is read in a list whose first option anchors the axis — spelling *předem* on every chip was measured to cost a third row on a phone.

⚠ **Two defects are recorded here rather than repaired.** `POST /api/electricity/periods` with a negative `invoiced_vt_dkwh` still returns **500** instead of 422, and electricity's `limit` still falls back where the shared helper clamps — both v8's to repay, both untouched by this pass and by v11 (D311).

---

## V10-17. As built — crash reporting and the feedback door (built 2026-09-03/04, no API change)

Reconstructed from [#47](https://github.com/kareltilcer/ws-tilcer-home/pull/47) and [#48](https://github.com/kareltilcer/ws-tilcer-home/pull/48); see §V10-16's opening note for why this section is late and why it is numbered here. Built against `ws-tilcer-status`'s own `docs/integration.md` and `docs/widget.md`. **No endpoint, no migration, no schema** — OpenAPI stays at 0.14.0. Its only prior written record was `README.md`.

**`home` now reports to status.tilcer.cz**, from both halves, and the household can write to Karel from inside the app.

⚠ **The backend seam is the LOGGER, not a `Capture` beside each failure.** `internal/platform/statusreport.NewLogHandler` wraps the slog handler in `main` and forwards every record at `slog.LevelError`, so all 54 existing `logger.Error(...)` sites report with **none of them edited**. That works only because the curation already existed: this service reserves `Error` for a fault that should not happen and logs everything soft at `Warn` — `scheduler: job failed` is a Warn and `scheduler: job panicked` is an Error, twelve lines apart in one file. *"What belongs on a crash board"* is a question the code has been answering for ten versions, and one wrapper inherits every answer; the alternative is 54 places to forget on the 55th.

The forwarder reads three attr keys as what they are. `err` (or `panic`) joins the **message**, because the message is what the server fingerprints — `admin: load trigger rules` alone would collapse every distinct failure of that call into one group named after the call and not the fault. `stack` becomes the report's stack field, whose first frame contributes to the default fingerprint. ⚠ **That is why `httpx.Recover` must keep its `stack` attr**, and it is a constraint v11's panic guard inherits (D313).

⚠ **AND IT IS WHERE `Report.Context`'s "NOT USER CONTENT" RULE IS ACTUALLY DECIDED — at the `logger.Error` call, not in the handler.** Every attr of every Error record is forwarded verbatim, so the rule cannot be applied in the forwarder; the only place it can be applied is the call site, and the person writing one is not reading that file. Spelled out where it bites: an Error line's attrs travel to a board read by Karel's **admin** session, which is a different lock from the one on a member's private note. Log the id, the key, the count, the route — never the title, the body, the filename or the message. All 54 sites were audited at the time of writing and carry ids and opaque object keys. ⚠ **With one known edge, written down rather than rounded off:** `Recover` is mounted at the router root, above chi's `NotFound`, so it also covers the SPA catch-all — and home's browser paths carry slugged titles (`/poznamky/soukrome/<a private note's title>`). It takes a panic inside a static file handler to reach.

**Two events are `fatal`, both written by hand in `main`**: a panic that unwinds it, and a boot that returns an error — the crash-loop a failed migration produces — captured synchronously, because `os.Exit` skips defers and in-flight goroutines. Everything the forwarder sends is `error`: a panic an HTTP handler recovers from killed the request, not the process.

**In the browser**, uncaught errors and unhandled rejections. React 19 routes an uncaught render error through `window.reportError`, so the white screen a broken component leaves behind arrives too — without this app growing an error boundary it has deliberately never had.

**The feedback trigger is home's own control.** The widget's floating launcher is suppressed (`data-launcher="none"`): it sits 16 px from a corner and does not account for a host app's bottom navigation, and home has a 56 px thumb-tab bar under every width below 768. So the trigger is a button in the desktop sidebar footer beside *Světlý* and *Odhlásit se*, and the last row of the mobile *Více* sheet — both rendered only when the widget script has actually loaded. ⚠ **The widget loads inside the AUTHENTICATED shell while crash reporting installs before the first render**, and the pair is deliberate: a crash belongs to the app and must be caught before anything paints, including on the login screen; a report belongs to a person, and there is nobody to sign it with until somebody is signed in.

**What it costs when status is down: nothing.** Every path fails safe on both sides. An ingest error — 401, 404, 413, 429, a dead socket — is dropped in silence; `Capture` never blocks a request path; the Go client carries the server's own 60/min-burst-120 limit so an error storm cannot become a goroutine and a socket per log line; the browser caps itself far lower; and both halves honour a 429 by muting and **dropping**, never queueing. ⚠ **The price is that a wrong key looks exactly like a quiet week**, which is why the boot line says which state reporting is in, once — the precedent `HOME_MCP_ENABLED` follows (D317).

**Configuration.** Four variables — `STATUS_INGEST_URL`, `STATUS_INGEST_KEY`, `STATUS_ENVIRONMENT`, `STATUS_RELEASE` — deliberately **not** `HOME_`-prefixed, because they are the fleet-wide names every `ws-tilcer-*` service reads. ⚠ They are the only variables in this service read **twice**: `cmd/home` builds the reporter from them *before* `config.Load` runs, so that a configuration error or a failed migration is itself reportable, and two spellings of one variable would be a feature that is simply off with nothing anywhere saying so. `StatusEnv` maps `HOME_ENV`'s `production`/`development` onto status's `prod`/`dev`, because the SPA posts to the same site from the same deployment and defaults to that vocabulary — without the mapping one release reached one board under two names. ⚠ **`STATUS_RELEASE` is the only backend release identifier that exists**, it is free-form, and it defaults to empty; v11's `initialize` reports it when set and the string `"dev"` otherwise, rather than inventing a build stamp (D278).

**#48 corrected the trigger's hint.** *"Napište Kájovi — i se snímkem obrazovky"* named a person and an attachment rather than an outcome, and narrowed the door: somebody with an idea rather than a fault was told the thing behind this row is a message to Kája about a screenshot. It now reads *"Nahlaste problém či nápad na vylepšení"*, which is what the dialog behind it actually takes — its own kind picker offers Chyba / Nápad / Něco jiného. ⚠ **The spelling is `Nahlaste`, not `Nahlašte`**: `nahlásit` takes the same imperative as `přihlásit`, which `cs.ts` already spells `Přihlaste se`.

### Outstanding from this pass

- [ ] ⚠ **The status widget's own Send button is unreachable on a phone and cannot be fixed from this repo.** That dialog is the widget's, inside a **closed shadow root** with `all:initial` on its host, and status's V3-D55 refuses a `--sfb-*` host-theming contract. It is fixed in `ws-tilcer-status` and reaches home only when status's frontend is rebuilt.
- [ ] The wiring checklist's step 7 (*trigger a test error*) never exercises the attachment path step 5 warns about, and `widget.md` §11 says a same-origin test proves nothing for CORS. A missed R2 CORS policy is a **lost** attachment, not a silent one — the widget names the file it could not upload and status settles the row `missing` on the board — and it runs until the policy is fixed, for every attachment of every report.

---

## V11-1. Overview (delta)

**v11 adds no twelfth module. It adds a second front door to all eleven.**

Every version from v1 to v10.2 added a room to the house. v11 hands out a key: an **MCP server**, built into the same binary, that lets an AI assistant on Karel's own machine read the household's data and make the small, reversible changes a member would make from the browser.

The work is not in any module. It is in the four things Home has never had to answer:

1. **Who is calling, when there is no session cookie.** Home is Mode B — it hosts its own login and the browser carries no bearer token. v11 introduces the first bearer credential Home has ever accepted for its own data: a **personal access token**, minted in Nastavení, hashed like a session, revocable, and expiring on a schedule its owner picks.
2. **What a caller may see, when it is a program.** The answer chosen is *everything the member can see* — the private (`soukromé`) roots of v9 and the membership-scoped conversations of v10 included. The three access axes are unchanged; the token simply **is** the member.
3. **How forty verbs stay legible to a reader with no screen.** 144 REST paths cannot become 144 tools. v11 publishes **39**, each a real verb with a real schema, under a ceiling of 45.
4. **What the Log says afterwards.** Every write through a token is attributed to its owner **and marked**, so the Log can answer *"did I do this, or did the assistant?"* — the question that did not exist before v11 and cannot be answered retroactively.

**The largest piece of work is not the protocol.** MCP over Streamable HTTP is one POST handler, a JSON-RPC envelope and a dispatcher. The large pieces are:

- **The fifth registered catalog** (D276). The MCP host needs data from every module while importing none of them, and it reaches them the way the dashboard reaches widgets and the scheduler reaches metrics — through a contract. It is also a **new host map**, and the PRD's own arithmetic (D213) makes it the **seventh**.
- **The pool.** Home runs on **ONE** database connection. A browser makes one request per view; an MCP client fans out parallel tool calls by design. §V11-8 is the answer and it is not optional.
- **The read risk.** v11 is narrow on verbs and wide on reads: an agent can destroy nothing and can read the private notes. The whole risk of the version is therefore in §V11-4's leak table, not in its write list.

**Not in v11:** OAuth 2.1 and dynamic client registration, and therefore claude.ai and the phone (D322) · server-initiated notifications and resource subscriptions (D277) · sampling and elicitation · a stdio bridge binary · uploads of any kind · any chat write (D296) · anything destructive (D295) · the v9/v10 two-threshold merge, still owed and still not this version's.

---

## V11-2. Goals & Non-Goals (delta)

**Goals**

- **G1** — A Claude client on Karel's machine answers *"co mám dnes udělat?"* in one tool call, from live household data, in Czech, without a browser.
- **G2** — The same client adds a card, logs a meter reading, records a harvest or writes a note, and the change appears in every open browser immediately, through the module's existing service layer and websocket publish — **no second write path exists anywhere in v11**.
- **G3** — Every such write is attributed in the Log to its member **and** marked as having come through an assistant, with the token named.
- **G4** — A token is minted, named, scoped, seen and revoked by its owner in Nastavení, and revoked by an admin in Administrace, in under a minute, without a deploy.
- **G5** — The three access axes — household visibility, ownership (v9), membership (v10) — hold **identically** on the MCP path. No route, tool, resource or error message expresses another member's view.
- **G6** — A chatty assistant cannot degrade the browser for the rest of the household.

**Non-Goals**

- **NG1** — Being addable in claude.ai, or usable from a phone. That needs OAuth 2.1 with dynamic client registration and a consent screen; it is v12, and it is most of a version by itself (D322).
- **NG2** — Parity with the browser. v11 exposes what is safe and useful, not what exists (D295).
- **NG3** — A generated surface. A tool per endpoint would be always in sync, mechanical, and unusable (D293).
- **NG4** — Multi-tenancy, per-tool consent, or an approval queue. A token is trusted the moment it is minted; the control is *which verbs exist at all*.
- **NG5** — Repairing anything v11 touches on its way past. The electricity `limit` defect, the two delete confirmations, the two threshold mechanisms and item 43 are all still open and none of them is v11's (⚠ with the one narrow exception in D311, which is not a fix).

---

## V11-3. Users, Roles & Auth (delta)

**The token is a second credential for the same identity, and everything that follows is a consequence of that sentence.**

- A token belongs to exactly one member. Its **roles are never stored on it** — they are the owner's, read live and re-minted on the same threshold as a session (D287). A `reader`'s token is a reader; there is no way to mint a token with more authority than you hold, and no way for a token's authority to drift from its owner's.
- **The session cookie is not accepted at `/mcp`, and the bearer is not accepted anywhere else** (D279, D280). The two credentials never meet. A request arriving at `/mcp` with a cookie and no bearer is **401**, not "authenticated as the cookie's owner" — because `/mcp` sits outside the `/api` group and therefore outside the CSRF double-submit check, and a cookie-authenticated endpoint without CSRF is a hole any website can drive.
- **`refreshIdentity` runs on the MCP path, on the same `HOME_ROLE_REFRESH_MINUTES` threshold, with the same fail-closed decision** (D287). A closed account drops a session today; on the MCP path it **revokes the token row**. Skipping this looks like an optimisation and is a key to a house whose lock was changed.
- **Roles gate exactly as they do over HTTP.** There is no `httpx.RequireWrite` here, so the gate is `reqctx.CanWrite` — the service-layer half that exists precisely for *"a service reached from a job or another module"*. Every write tool calls the module's service, which is gated once; the host checks again before dispatch, so a write tool is gated twice, as over HTTP.
- **Admin-only surfaces stay admin-only.** `home_admin_status` and `home_activity`'s cross-member filters resolve the caller's roles, not the token's provenance.
- **The `modules` allowlist narrows and never widens** (D288). It is a convenience — *this token is for the garden* — not an access control, and the PRD says so out loud so nobody builds on it as one.

**Actor.** `reqctx.Actor` on the MCP path carries `UserID` = the member, `Roles` = the member's, `Type` = **`"user"`** (D290), and `Label` = `"<display name> · <token name>"` (D291). Every ownership check, membership check and `created_by` stamp in eleven modules keeps working with no change at all, which is the reason a separate identity per token was rejected.

---

## V11-4. Functional Requirements (v11)

### FR-M1 — The MCP endpoint

`POST /mcp` speaks JSON-RPC 2.0 over Streamable HTTP. `GET /mcp` returns **405** — v11 sends no server-initiated notifications (D277). Methods implemented: `initialize`, `tools/list`, `tools/call`, `resources/list`, `resources/templates/list`, `resources/read`, `prompts/list`, `prompts/get`, `ping`. `MCP-Protocol-Version` is honoured and echoed; an unsupported version is refused at `initialize` with the supported one named.

`initialize` without a bearer returns server capabilities **and nothing else**; every other method requires the bearer (D312), so an unauthenticated caller cannot read the tool list and learn the household's shape.

### FR-M2 — The fifth catalog

`platform/mcp` defines the contract and assembles the registry at composition. A module opts in by implementing the **optional** `mcp.Source` (D276) — never by a change to `registry.Module`:
...
type Provider interface {
    ...
    Search(ctx context.Context, q Query) ([]Hit, error)          // nil ⇒ nothing to find
    Resources() []ResourceTemplate
    Read(ctx context.Context, uri string) (Content, error)
}
```

The host never queries a module's tables. `internal/arch` stays green.

⚠ **A provider with no tools is not an empty provider.** `dashboard` implements `Source` not at all; **`logging` implements it with an empty `Tools()` and a real `Search`** over `audit_events_fts` (FR-M6). Contributing nothing to a catalog is ordinary here — five of the eleven modules publish no widget either (`admin`, `logging`, `chat`, `electricity`, `dashboard`).

### FR-M3 — Tokens

Mint, list, rename, revoke. The secret is generated as `hmcp_` + base64url of 32 random bytes, **shown exactly once**, stored only as SHA-256, and identified thereafter by its 8-character prefix (D284). Expiry is chosen at mint — 30, 90, 365 days or never — and **does not slide** (D285). `last_used_at` and `last_used_ip` are recorded under the same one-hour `touchGranularity` sessions use, so a forty-call conversation writes to the row once (D286). At most `HOME_MCP_MAX_TOKENS_PER_USER` live tokens per member (D289).

### FR-M4 — The tool surface (39, ceiling 45)

**Core (7):** `home_whoami` · `home_search` · `home_get` · `home_today` · `home_metrics` · `home_lists` · `home_activity`.

**Per module (32):** todo 5 · events 4 · notes 4 · documents 3 · finance 3 · garden 6 · electricity 4 · chat 2 (read-only) · admin 1 (read-only). dashboard and logging contribute none.

`home_today` is composed **entirely from the metrics and lists catalogs** (D310) — 19 metrics and 16 lists, personal ones resolved per recipient — so the single most-called tool in the system touches no module table and adds no cross-module read. ⚠ **It therefore carries no unread count**: D252 kept chat out of both catalogs, and an unread figure here would be the one exception that breaks the property. Unread rides on `home_chat_conversations`, where it is free.

### FR-M5 — Writes

Create and update only. **Delete, publish, purge, upload, membership change, admin mutation, season close/reopen and dashboard layout are absent from the tool list** — not gated, not confirmed, *absent*, so the model cannot propose them (D295). Every write goes through the module's existing service, so it audits in the same transaction and publishes to `/ws` with the same `origin` (G2). **No tool carries `destructiveHint`, and a test asserts none ever does** (D294).

### FR-M6 — Cross-module search

`home_search` fans out to each module's `Search`, viewer-scoped, with a **per-module** budget (D301). The host merges and orders by *(exact title match, then recency, then module order)* and **says so in the tool description** (D300): bm25 scores from five separate FTS5 indexes are not comparable, and presenting a merge as a ranking produces confident nonsense. There is no cross-module SQL and there cannot be (D299).

⚠ **A provider whose ctx carries no actor returns an error — never `AnyScope`, never empty** (D302). This is v9's exact trap: its build found two surfaces no review had listed (the preview worker and the image GC) because a background job has no actor and someone reached for the unscoped load. An MCP tool call has an actor only *after* the token resolves.

### FR-M7 — Resources

Three templates: `home://notes/{path}` · `home://documents/{path}` · `home://chat/{conversation}/attachments/{id}`. ⚠ **`{path}` is the SPA splat verbatim, with no scope segment** — shared unprefixed, private prefixed `soukrome`, as `lib/scope.ts` parses a URL; there is no `sdilene` and inventing one would be the only place in the app naming the shared root. Unambiguous because `soukrome` is reserved at both shared roots (D185). Listings are viewer-scoped; **every read re-checks access from scratch, because a URI is not a capability** (D308). A file that **is** text (`text/plain`, `text/markdown`, `text/csv`) is served as text; PDFs and images are served as content. ⚠ **Nothing extracts text from anything else** — `home-gotenberg` converts Office files to **PDF**, never to text, so a .docx reaches the model as a rendered PDF or not at all. No new conversion path and no `platform/preview` (D227 stands). Results are capped at `HOME_MCP_MAX_RESULT_KB` (D307).

### FR-M8 — Prompts

Five, Czech-named, Czech-bodied, English tool calls underneath: `/co-mě-čeká` · `/nákup` · `/večeře-z-toho-co-máme` · `/měsíční-uzávěrka` · `/zahrada-týden` (D309).

### FR-M9 — Attribution and the Log

Every audit event written on the MCP path carries `via='mcp'` and `via_token_id` (D290), and its `actor_label` names the token (D291). The Log browser gains one filter, **Přes asistenta**, and one chip on the row (D292).

### FR-M10 — Two reads that are audited

**`chat.read` and `notes.private.read` write an audit event when the reader is a token** (D297) — the second and third reads ever audited in Home, after v9's `admin.private_items.view`. The event records **that a token read a conversation or a private note, never what was in it**: `entity_id` is the container, `summary` names it, `meta` carries the token id and a count. No body, no snippet — the Log is not a second copy of the chat, which is the whole reason D231 exists.

### FR-M11 — The leak table (17 rows)

Reproduced from the brief §6, with the closure for each. ⚠ **Rows 7, 8 and 11 are one failure wearing three hats — a read that forgot who was asking — and are reviewed together.**

| # | Surface | Closed by |
|---|---|---|
| 1 | `/mcp` accepts the session cookie ⇒ any website drives the tool surface cross-origin with no CSRF | Bearer only; a cookie-only request is 401 (D280) |
| 2 | `/mcp` unmatched falls through to the SPA — `httpx.NewRouter`'s `NotFound` sends every non-`/api/`, non-`/ws` path to `index.html` — returning **HTML with a 200** | `/mcp` joins `/ws` in the exclusion (D282) |
| 3 | DNS rebinding from a local page | `Origin` validated against `HOME_ALLOWED_ORIGINS`; absent `Origin` allowed only with a valid bearer (D281) |
| 4 | A revoked token works while a cached lookup lives | No positive cache; one indexed lookup per call (D287) |
| 5 | A token outlives its owner's account being closed in auth | `refreshIdentity` on the MCP path, fail-closed, revokes the row (D287) |
| 6 | A token grants more than its owner has | Roles are the owner's, re-minted; `modules` narrows only (D287, D288) |
| 7 | `home_activity` returns raw `audit_events` including another member's private-item summaries and ids | `audit.Redact` / `RedactRendered` on the MCP path (D298) |
| 8 | `home_search` returns another member's private note via an `…AnyScope` load | Actor-less ctx is an error, per module (D302) |
| 9 | 404-never-403 breaks in the error mapping — a tool answering *"forbidden"* leaks what *"not found"* hides | One mapper; 403 on an ownership/membership surface maps identically to 404 (D303) |
| 10 | A resource URI treated as a capability | Fresh access check on every read (D308) |
| 11 | `resources/list` enumerates what the caller may not read | Viewer-scoped listing; a private root appears only for its owner (D308) |
| 12 | A chat read leaves no trace | `chat.read` audit event, container never body (D297) |
| 13 | Tool descriptions leak household structure to an unauthenticated caller | Everything but `initialize` requires the bearer (D312) |
| 14 | The token is logged — request log, error, or a crash report to status.tilcer.cz | Never leaves the auth path; `config.Redacted()` covers it; **`Authorization` stripped from the `statusreport` handler** (D313) |
| 15 | A tool result large enough to be a denial-of-service against the model's context | `HOME_MCP_MAX_RESULT_KB` (D307); ranged resource reads |
| 16 | An MCP-created event pushes to every member with no explanation | Nothing suppressed — a created task *should* notify — but its audit trail carries `via='mcp'` (D321) |
| 17 | A leaked `*sql.Rows` in a new provider deadlocks the **next query in the whole process**, browser included | `appdb.Collect` everywhere; review item + load test (§V11-8) |

---

## V11-5. Data Model (v11)

**Two migrations, no new block.**

**`02005_mcp_tokens.sql`** — platform block, beside `02001_sessions`:
...
CREATE TABLE mcp_tokens (
    ...
    token_hash    TEXT NOT NULL UNIQUE,           -- SHA-256 of the secret; never the secret
    prefix        TEXT NOT NULL,                  -- first 8 chars, for identification in the UI
...
⚠ **`expires_at` is written once and never touched again.** The resemblance to `sessions` is a trap: a session slides on use (bounded by `touchGranularity`); a token that renewed itself on every call would be a token that never expires, which is the option that was not chosen.

**`01003_audit_via.sql`** — logging block, two columns and a partial index:
...
⚠ **This is deliberately NOT a fourth `actor_type`, and the reason is a hazard, not a preference** (D290).

```sql
actor_type TEXT NOT NULL CHECK (actor_type IN ('user', 'system', 'service')),
```

`reqctx.Actor.Type` is written into that column by `audit.Sink` (`sink.go:105`, substituting `system` when it is empty). **SQLite cannot ALTER a CHECK**, so a fourth value means rebuilding `audit_events` — the table holding **every mutation the household has ever made**, and the parent of `audit_changes`, which `REFERENCES audit_events (id) ON DELETE CASCADE`. That is `#46`'s `04002` hazard at far worse scale: with `foreign_keys=ON`, dropping the parent performs an implicit DELETE, fires the cascade, and takes the household's entire change history **while reporting success**. `04002` needed 229 lines and a rename-never-drop procedure to widen one enum on a small table. `ADD COLUMN` is cheap, has no rebuild, no cascade and no down-migration risk, and the partial index costs nothing on the 99% of rows where `via IS NULL`.

⚠ **A third migration is deliberately absent.** No table records what a token *did*; that is what `audit_events` is for, and a second log would be a private, unsearchable, un-redactable copy of the first — D225's reasoning, applied to a new temptation.

---

## V11-6. API Surface (v11)

Two surfaces, and only one of them is OpenAPI.

### The REST half — `openapi.yaml` **0.15.0**, additive, session-authorized, CSRF-protected

Four new paths carrying six operations, taking the contract from 144 to **148**; six new schemas, one new query parameter, and two new nullable fields on `AuditEvent`.

| Group | Paths |
|---|---|
| My tokens | `GET·POST /api/mcp/tokens` · `PATCH·DELETE /api/mcp/tokens/{id}` |
| Admin | `GET /api/admin/mcp/tokens` · `DELETE /api/admin/mcp/tokens/{id}` |

- `POST /api/mcp/tokens` is **the only response in Home that ever contains a secret**, and it contains it once. The schema is `McpTokenCreated` — the `McpToken` fields plus `secret`, marked `writeOnly: false` and documented as unrepeatable. Every other read returns `McpToken`, which has no secret field at all, so a client cannot accidentally log one it never received.
- `DELETE` is a **revoke**, not a delete: it stamps `revoked_at` and the row stays, because `via_token_id` on an audit event must still resolve to a name years later. A revoked token is listed with its revocation date and cannot be un-revoked.
- `PATCH` changes the **name and the module allowlist only**. Not the expiry — extending a token's life is minting a new one, which is a decision a member should have to take deliberately.
- `GET /api/admin/mcp/tokens` returns every member's tokens with owner, name, prefix, scope, created, expiry and last-used. **It contains no secret and no hash, and there is no admin mint** (D316): an admin may see that a key exists and take it away, and may not make one in someone else's name. This is D240's shape — *names and sizes, and no way in* — applied to credentials.
- **`AuditEvent` gains `via` and `via_token_id`**, both nullable, on the existing Log endpoints; `GET /api/logs` gains a `via` filter parameter.

⚠ Writing 0.15.0's CHANGELOG entry on top of a **0.14.0 that has none** is how a changelog stops being readable. The `#46` and `#47`/`#48` entries and the Nextcloud re-sync are **v11's first commit, not its last**.

### The MCP half — not OpenAPI, and it needs its own artifact

`openapi.yaml` describes HTTP resources; MCP is JSON-RPC with JSON Schema per tool. Forcing one into the other produces a document that is wrong in both directions. v11 ships instead (D320):

- **`backend/mcp-manifest.json`** — the full tool, resource and prompt manifest, **generated by a test from the live registry, committed, and diffed in review**, so a tool's schema cannot change without somebody seeing it. This is the treatment `platform/storage`'s table declarations got, for the same reason.
- **`HANDOFF-13-mcp.md`** — the prose: the contract, the dispatcher, the token lifecycle, the leak table with its tests.

**Error mapping** (D303): `httpx.APIError` → a JSON-RPC result with `isError: true` and the Czech detail. **`403` on an ownership- or membership-scoped surface maps to a result byte-identical to `404`** — 404-never-403 is a property of the *answer*, not of the status line, and a tool that says *"forbidden"* leaks precisely what *"not found"* hides. Protocol-level failures (bad method, malformed params, unknown tool) are JSON-RPC errors; a tool that ran and refused is an `isError` result, because a model retries the first and reads the second.

---

## V11-7. Frontend (v11)

Two surfaces, one page each, no new route paths beyond the existing Czech ones.

**Nastavení → Asistenti (MCP)** — a **fourth** `<section>` in `platform/settings/NastaveniPage.tsx`, after *Notifikace*, *Vzhled a účet* (D273) and *Aplikace*. A list of the member's tokens — name, prefix, **Rozsah**, **platnost do**, **naposledy použito** — and *Vytvořit token*. The mint dialog asks for a name, an expiry (30 / 90 / 365 dní / bez omezení) and a module selection defaulting to all.

⚠ **The one-time reveal is a pattern Home has never drawn** (D315). The secret appears once, in a dialog that cannot be reopened, with a copy button, a *Token se zobrazí jen jednou. Zkopírujte si ho teď.* line, and **no dismissal by outside-click or Escape** — only an explicit *Hotovo*. A 48-character string at 375 px wraps; it is rendered in a monospace block that scrolls horizontally inside its own container, never one that widens the page.

**Administrace → Asistenti → Tokeny.** A **third level-1 group** in the two-level tab strip — beside *Notifikace* (4 tabs) and *Správa úložiště* (**3** since D263: `storage`, `private`, `limits`) — holding one tab, taking the strip from seven tabs to eight. ⚠ **D202's own text says "six tabs" and is already stale by one**; do not copy the number out of it. Every member's tokens, owner named, with *Odvolat*. No secret, no hash, no mint (D316).

**Log browser.** One new filter, **Přes asistenta**, and one chip on the row where `via='mcp'`, reading the token's name.

⚠ **v10.2's lesson applies before a line is drawn: drift lives in shared chrome, and no per-module review sees it.** The phone's app header existed for nine releases in no artboard of any version. These two surfaces get their phone artboards in the same bundle as their desktop ones, not after.

⚠ Both surfaces use `cs.ts`. They do not repay item 43, and they do not add to it.

---

## V11-8. Non-Functional Requirements (v11)

**The pool is the NFR.** `appdb` caps the connection pool at **one**, deliberately, and the whole backend is written around it — `appdb.Collect` closes rows precisely because *a leaked `*sql.Rows` deadlocks the next query*.

An MCP client breaks the assumption that produced that cap. Claude issues parallel tool calls routinely; six at once is ordinary. Six tool calls are six-plus queries with nothing pacing them, and the household's dashboard is behind them in the same queue.

- **NFR-1 — A semaphore of 2 per token** (D304). The third concurrent call waits. This is back-pressure, not a rate limit, and it is what keeps a chatty agent from being a self-inflicted outage.
- **NFR-2 — A hard per-call deadline**, `HOME_MCP_CALL_TIMEOUT_SEC` (20), on the ctx, so a pathological search cannot hold the connection after the client has given up (D305).
- **NFR-3 — A per-token rate limit**, `HOME_MCP_RATE_PER_MIN` (120). ⚠ **`auth`'s `rateLimiter` does not fit as it stands** and the PRD says so rather than leaving the build to find out: it is a **fixed-window** counter, **unexported**, and it counts **failures only** — `allowed` deliberately records no attempt, because only failed logins are counted. A call-rate limit counts successes (D306).
- **NFR-4 — Result cap**, `HOME_MCP_MAX_RESULT_KB` (256), enforced by the host, not by each provider (D307).
- **NFR-5 — One token lookup per call**, on a hashed unique index, with no positive cache, so a revocation takes effect on the next call (D287).
- **NFR-6 — `last_used_at` at most once per token per hour** (D286), or every read tool becomes a writer.

⚠ **This wants a load test, not a unit test.** The criterion is *"twenty concurrent tool calls, and a browser request served throughout"* — the shape of thing v10.2 proved a green suite does not tell you. ⚠ And **`go test -race` is unavailable on the windows/arm64 dev host**, so the concurrency review is by reading, on the one surface where that is least comfortable.

**Observability.** `/mcp` requests go through the existing `httpx.Logger` and carry a request id like everything else; the tool name is a log attribute. ⚠ **`Authorization` must be stripped before the `statusreport` slog handler sees it** (D313) — that handler lifts attrs into the crash report, and the crash board groups on the first frame.

---

## V11-9. Configuration (v11)

Six new variables, all `HOME_`-prefixed, all defaulted, **none secret**.

| Variable | Default | Notes |
|---|---|---|
| `HOME_MCP_ENABLED` | `true` | ⚠ Defaults **on** on purpose (D317): the real gate is that a human must mint a token in the UI, and a flag defaulting to off is a second thing to forget on a surface whose failure mode is silence. Says its state **once at boot**, the way `statusreport` does, and for the same reason — a disabled server and a misconfigured client look identical from the client. |
| `HOME_MCP_RATE_PER_MIN` | `120` | Per token. |
| `HOME_MCP_CALL_TIMEOUT_SEC` | `20` | Per call, on the ctx. |
| `HOME_MCP_MAX_RESULT_KB` | `256` | Per tool result and per resource read. |
| `HOME_MCP_MAX_TOKENS_PER_USER` | `10` | Live (non-revoked, non-expired) tokens. |
| `HOME_MCP_SEARCH_LIMIT` | `10` | **Per module**, not globally (D301). |

⚠ **Bounds follow v10's precedent, not v9's.** The three v10 session windows gained upper bounds; the two older ones **clamp with a loud `CONFIGURATION CORRECTED` warning** rather than refusing, because aborting `Load` would crash-loop the container on the deploy that lands them. v11's six are all new, so they **refuse at both ends** — nothing is upgrading into them.

⚠ **Deployment is not code** (D318). `home.tilcer.cz` is **two Coolify apps sharing one origin**, routed by path — `home-backend` on `/api` and `/ws`, `home-frontend` on the catch-all; `home-gotenberg` is a third app but internal-only, with no public domain. `/mcp` is the backend's **third path route**, to **7999**, and **Strip Prefix stays off** as it has since v1. Without the route, `/mcp` reaches the Nginx SPA and returns `index.html` — the same symptom as leak row 2, from a different cause, which is what will make it hard to diagnose. **Check both.**

⚠ **No new secret and no new outbound dependency.** v11 adds nothing to `LITESTREAM_*`, nothing to the `STATUS_*` set, and no third-party endpoint.

---

## V11-10. Decisions (D275–D322)

- **D275 — v11 adds no module.** It adds a second front door: `platform/mcp`, mounted at `POST /mcp`, OpenAPI **0.15.0**, build guide `HANDOFF-13-mcp.md`, migrations `02005` and `01003`. Ten of the eleven modules gain a provider — nine publishing tools, `logging` publishing only `Search`, `dashboard` none at all — and none gains a table.
- **D276 — The MCP host is the FIFTH registered catalog**, and it copies the shape of the third and fourth exactly: an **optional** `mcp.Source` interface, type-asserted at composition, assembled into a `*mcp.Registry` — **never part of `registry.Module`**. D56's rule, restated at every catalog since: adding a capability must not change the contract every module implements. The host reaches module data through the contract and never through a table, which is what keeps `internal/arch` green with a sixth cross-module consumer in the tree.
- **D277 — Streamable HTTP, `POST /mcp` only. `GET /mcp` returns 405.** v11 sends no server-initiated notifications and offers no resource subscriptions. The hub exists and could feed them; nothing asked for it, and it would be the first thing in Home that pushes to a non-browser. Declining is one line; building it is a version.
- **D278 — Hand-rolled JSON-RPC; no MCP SDK dependency.** Home has **13 direct dependencies**, all mainstream and pinned, and already hand-rolls a websocket hub over `coder/websocket`. Streamable HTTP is one POST handler and an envelope; `github.com/modelcontextprotocol/go-sdk` is pre-1.0 and moves faster than a household app should. ✅ **SETTLED 2026-09-06, no longer provisional.** The cost accepted with it is stated rather than glossed: **protocol-version drift is ours to track**, and the `initialize` handler is where that shows up first. (The Go module proxy is also blocked from a Cowork session, so a new dependency could not have been verified from there anyway.)
- **D279 — `/mcp` is mounted OUTSIDE the `/api` group**, with its own middleware chain: request id, logger, recover, then bearer auth. No `SessionMW`, no `CSRFMW`, no Origin allowlist inherited — each is decided here rather than assumed.
- **D280 — The session cookie is not read at `/mcp` at all.** A request with a cookie and no bearer is **401**. `/mcp` has no CSRF double-submit check because it has no cookie to protect; accepting one would make it the only cookie-authenticated, CSRF-unprotected endpoint in Home, drivable cross-origin by any website the member visits.
- **D281 — `Origin` is validated against `HOME_ALLOWED_ORIGINS`** — the list CSRF already uses. A request with **no** `Origin` is allowed, because a CLI has none, but only with a valid bearer. This is the DNS-rebinding defence the MCP specification requires of HTTP servers, and it costs one shared helper.
- **D282 — `/mcp` joins `/ws` in the router's SPA exclusion.** `httpx.NewRouter`'s `NotFound` sends every non-`/api/`, non-`/ws` path to `index.html`, so without this a mistyped MCP path returns **HTML with a 200** and the client reports a protocol error against a working server. One line, and it is the first thing that will be got wrong.
- **D283 — `mcp_tokens` is a platform table at block `02005`**, beside `02001_sessions`. It belongs to no module: `auth` writes it, the MCP host reads it, `admin` lists it, and none may import another.
- **D284 — The secret is `hmcp_` + base64url(32 random bytes), shown once, stored as SHA-256, identified by an 8-character prefix.** The literal prefix is not decoration — it makes a leaked token greppable in a config file, a paste and a log, which is the only cheap mitigation that exists for a credential that lives on a laptop.
- **D285 — Expiry is chosen at mint (30 / 90 / 365 / never) and does NOT slide.** ⚠ The resemblance to `sessions` is a trap: a session extends on use; a token that renewed itself on every call would never expire, which is the option that was not chosen. `PATCH` cannot change it — extending a token's life is minting a new one.
- **D286 — `last_used_at` is written at most once per token per hour**, reusing `auth`'s `touchGranularity`. Without it a read-only conversation of forty tool calls does forty writes to one row, on a one-connection pool.
- **D287 — Roles are never stored on the token; `refreshIdentity` runs on the MCP path with the same threshold and the same fail-closed decision, and on failure it REVOKES the token row.** A bearer that skips the re-mint is a key to a house whose lock was changed. There is no positive cache anywhere in the lookup, so a revocation takes effect on the very next call.
- **D288 — The `modules` allowlist narrows and never widens, and it is a convenience rather than an access control.** Said out loud in the PRD so nobody builds a security property on it. `[]` means everything the owner can see.
- **D289 — At most `HOME_MCP_MAX_TOKENS_PER_USER` (10) live tokens per member.** Not a security boundary — a hygiene one: a list nobody can read is a list nobody revokes from.
- **D290 — Attribution rides on two new NULLABLE COLUMNS, not on a fourth `actor_type`.** `actor_type` is `CHECK`-constrained; SQLite cannot ALTER a CHECK; `audit_events` is the largest table in the database and the parent of `audit_changes` with `ON DELETE CASCADE`. Rebuilding it is `#46`'s `04002` hazard at far worse scale — an implicit DELETE fires the cascade and takes the household's entire change history **while reporting success**. `ALTER TABLE ADD COLUMN` has no rebuild, no cascade and no down-migration risk. ✅ **CONFIRMED by Karel 2026-09-06.** The fourth `actor_type` is **not wanted**, not merely deferred — so ⚠ **anyone who later finds `actor_type` lacking an `agent` value has found a decision, not an oversight**, and `TestActorTypeStaysThree` asserts the CHECK still names exactly `user | system | service`. (This is v9's `TestReplicaIsDeclinedNotUnimplemented` pattern: a declined thing needs a test, or the next reader repairs it.)
- **D291 — `Actor.Type` stays `"user"` and `Actor.Label` becomes `"<display name> · <token name>"`.** Every ownership check, membership check and `created_by` stamp in eleven modules keeps working unchanged — which was the whole reason a separate identity per token was rejected. The Log already renders `COALESCE(actor_user_id, actor_label, actor_type)`.
- **D292 — The Log gains one filter (*Přes asistenta*) and one row chip**, on a partial index over `via IS NOT NULL`, so the filter is a seek and costs nothing on the 99% of rows where it is null.
- **D293 — 39 tools, ceiling 45, named `home_<module>_<verb>` in English snake_case.** 144 paths cannot become 144 tools; a model's selection accuracy collapses well before that and the list alone would spend the context budget before the first question. **Every tool past 45 must retire one.**
- **D294 — Nothing destructive is exposed, and a test asserts no tool carries `destructiveHint`.** The absence is the mechanism: a gated destructive tool is one the model still proposes and the member still approves at 23:40.
- **D295 — Delete, publish, purge, upload, membership change, admin mutation, season close/reopen and dashboard layout are ABSENT, not gated.** Each for its own reason: soft-delete is a courtesy and not an undo the agent understands; publish is one-way (D182) and moves an item out of a private root; a folder delete cascades a subtree in one transaction — and the two confirmations do not even count the same thing; finance has no soft-delete at all (D87); uploads are bytes over JSON-RPC against a 50 MB cap for a use case nobody asked for; the dashboard is the member's screen, not their data. ⚠ **Season close is the strongest of them and the easiest to misread as harmless:** `closeOutTasks` marks every still-open task `TaskSkipped` — it destroys work rather than regenerating it — and the closed year becomes the rotation history C3 and C8 read; reopening *"přepisuje se tím historie střídání plodin"* and is garden's **only admin-gated action**.
- **D296 — Chat is READ-ONLY over MCP, for two independent reasons.** First, the blast radius is another person: a message is delivered to a phone with a push, under your name, and no operation unsends it. Second, **it could not have been implemented cleanly anyway** — v10 decided chat messages are not audited (D231/D256, guarded by `TestChatMessagesAreNotAudited`), so an MCP-posted message would be the only write in the system with nowhere to put the `via` marker. Honouring the attribution answer for chat would have meant reversing D231.
- **D297 — `chat.read` and `notes.private.read` are audited when the reader is a token** (✅ both confirmed 2026-09-06; *chat only* was offered and declined) — the second and third reads ever audited, after v9's `admin.private_items.view`. **The event records the container, never the content:** `entity_id` is the conversation or note, `summary` names it, `meta` carries the token id and a count. Without this, *"what has the assistant seen?"* has no answer at all — unacceptable for a surface deliberately opened onto private notes and message bodies. ⚠ It does **not** reverse D231: no message row is ever written to the Log, and no body or snippet appears anywhere.
- **D298 — `home_activity` applies `audit.Redact` / `RedactRendered`.** The Log *page* redacts (D201, four phrases by `entity_type`); an MCP tool reading the raw table would bypass every one of those rules, for every member's private items, in one call. ⚠ This is the v9 trigger-listener gap — *the code is correct and nothing tests it* — repeating in a brand-new consumer, and the acceptance criterion asserts it against a **second member's** private item, not a fixture the caller owns.
- **D299 — The cross-module search fans out; there is no cross-module SQL and there cannot be.** Each module answers over its own index, viewer-scoped, with its own LIMIT; the host merges.
- **D300 — Ranking is a documented MERGE, not a score.** Order is *(exact title match, recency, module order)*, and the tool description says so. bm25 values from five separate external-content FTS5 indexes are not comparable, and a global "relevance" assembled from them is confident nonsense. A module may pre-order its own rows; it may not claim a global score.
- **D301 — The search budget is PER MODULE** (`HOME_MCP_SEARCH_LIMIT`, 10), and the response reports each module's count. A search returning 40 chat messages and no notes because chat is chattier is a worse answer than 8 of each.
- **D302 — A provider given a ctx with no actor returns an ERROR — never `AnyScope`, never empty — and each module has a test with a deliberately actor-less context.** v9's build found two surfaces no review had listed (the preview worker, the image GC) for exactly this reason: a background job has no actor, a viewer-scoped read returns nothing, and someone reaches for the unscoped load. An MCP call has an actor only after the token resolves.
- **D303 — One error mapper: `httpx.APIError` → JSON-RPC.** A **403 on an ownership- or membership-scoped surface maps to a result byte-identical to a 404** — 404-never-403 is a property of the answer, not the status line. Protocol failures are JSON-RPC errors; a tool that ran and refused is an `isError` result, because a model retries the first and reads the second.
- **D304 — A semaphore of 2 concurrent calls per token.** Back-pressure, not a rate limit. The pool is **one connection**; six parallel tool calls queue, and the household's dashboard queues behind them.
- **D305 — A hard per-call deadline on the ctx** (`HOME_MCP_CALL_TIMEOUT_SEC`, 20), so a pathological query cannot hold the one connection after the client has given up.
- **D306 — The rate limit is keyed by token id, and it is NOT `auth`'s `rateLimiter`.** ⚠ That type is a **fixed-window failure counter**, unexported, whose `allowed` deliberately records no attempt because only failed logins are counted — three reasons it cannot be reused as it stands, none of them obvious from its name. Either promote it to `platform/` with a counting mode, or write a small one in `platform/mcp`. **Decide before the build starts**, because "reuse the existing limiter" is the note a builder will act on without reading it.
- **D307 — Results are capped at `HOME_MCP_MAX_RESULT_KB` (256), enforced by the HOST**, not by each provider — a cap each of eleven providers must remember is a cap that is wrong in at least one of them.
- **D308 — A resource URI is not a capability.** `resources/list` is viewer-scoped and a private root appears only for its owner; `resources/read` re-checks access from scratch on every call. A URI pasted from another member's context is a 404, not a read.
- **D309 — Five prompts, Czech-named and Czech-bodied.** They are the cheapest thing in v11 and the place household knowledge actually lives: `/večeře-z-toho-co-máme` is `garden.storage` + `garden.harvests` + a sentence about what this family eats, and it is worth more than any three tools.
- **D310 — `home_today` is composed entirely from the metrics and lists catalogs**, never from module tables. The most-called tool in the system therefore adds no cross-module read and no new query — and personal-scope metrics resolve per recipient, as they already do for scheduled summaries.
- **D311 — Every write tool validates its own inputs before it reaches a service.** The rule exists because of electricity: a negative `invoiced_vt_dkwh` / `invoiced_nt_dkwh` surfaces as a **500 instead of a 422**, and an agent **retries a 500 and gives up on a 422**, so a household typo returning the wrong code turns a mistake into a loop. ⚠ **Those fields are on `electricity_periods`, and v11 exposes no periods tool** — so this particular defect is avoided by absence, not by care, and the rule is written down for the other fifteen write tools rather than for it. ⚠ **v11 fixes nothing in the REST surface**: the periods 500 and electricity's non-clamping 100/500 `limit` (it adopts neither `httpx.Limit` nor `appdb.ClampLimit`) are both still v8's to repay, and must not be recorded as repaired.
- **D312 — Everything but `initialize` requires the bearer.** An unauthenticated `initialize` returns server capabilities and nothing else, so `tools/list` cannot be used to learn the household's shape.
- **D313 — The token never leaves the auth path.** `config.Redacted()` covers the new variables; **`Authorization` is stripped before the `statusreport` slog handler sees it**, because that handler lifts attrs into the crash report and the crash board groups on the first frame.
- **D314 — Two frontend surfaces: a FOURTH section in Nastavení — *Asistenti (MCP)* — and a THIRD level-1 group in Administrace, *Asistenti* → *Tokeny*.** Still v7's pattern at level 1, so D202 holds — though ⚠ **D202's "six tabs" is stale**: D263 made *Správa úložiště* three tabs, so the strip is at seven today and v11 takes it to eight. No new route path; Nastavení keeps its full row in the phone's *Více* sheet, unchanged by D273.
- **D315 — The secret is revealed once, in a dialog that cannot be dismissed by outside-click or Escape** — only an explicit *Hotovo* — with a copy button and a monospace block that scrolls inside its own container. ⚠ A pattern Home has never drawn, and a 48-character string is the worst thing yet asked to fit 375 px.
- **D316 — An admin may see that a token exists and revoke it; an admin may not read a secret, a hash, or mint a token in another member's name.** D240's shape — *names and sizes, and no way in* — applied to credentials.
- **D317 — `HOME_MCP_ENABLED` defaults TRUE and logs its state once at boot.** The real gate is that a human must mint a token in the UI; a flag defaulting to off is a second thing to forget on a surface whose failure mode is silence — a disabled server and a misconfigured client look identical from the client. The boot line is `statusreport`'s precedent, for the same reason.
- **D318 — `/mcp` needs a NEW Coolify path route to the backend on 7999 — its third, after `/api` and `/ws` — and Strip Prefix stays OFF.** (`home.tilcer.cz` is two public apps sharing the origin; `home-gotenberg` is internal-only.) ⚠ Without it, `/mcp` reaches the Nginx SPA and returns `index.html` — the same symptom as D282's bug from a different cause, which is what will make it hard to diagnose. Check both.
- **D319 — `internal/arch/mcp_completeness_test.go`, and a committed golden manifest.** The tool catalog is the **seventh host map** on the PRD's own arithmetic (D213's six). ⚠ v10.2's `DESKTOP_FOOTER_ROUTES` split is an eighth candidate the PRD never numbered; settle it in the same pass, because *"the four were never four"* is now a recurring sentence. The guard asserts: unique tool names; every tool's module registered; every declared resource template readable; no tool carrying `destructiveHint`; and the full manifest matching `backend/mcp-manifest.json`, so a schema change is visible in review. `platform/storage`'s table declarations got this treatment because a real bug shipped green — *"it was found by opening the page"* — and there is no page to open here.
- **D320 — `openapi.yaml` covers the REST token endpoints only; the MCP surface gets `mcp-manifest.json` + `HANDOFF-13-mcp.md`.** OpenAPI describes HTTP resources and MCP is JSON-RPC with JSON Schema per tool; forcing one into the other produces a document wrong in both directions. ⚠ The manifest is **generated by the test**, never hand-written — the **seventh** host map (D213's six, plus this one) is not also going to be hand-maintained.
- **D321 — Push is NOT suppressed for MCP-originated writes.** A task created for the household should notify the household; the explanation lives in the audit trail (`via='mcp'`), not in silence. A notification with no matching Log row would be the confusing outcome, not this one.
- **D323 — v11 ships as THREE pull requests, not one.** **PR 1** — `platform/mcp`, both migrations, the token store and its four routes, bearer auth + Origin + SPA exclusion + semaphore + deadline + limiter, attribution end to end, and **only three providers: `todo`, `events`, `notes`**. **PR 2** — the remaining six providers plus `logging`'s search-only one, the cross-module search and its merge, resources, and the two read-audit actions. **PR 3** — the two frontend surfaces, prompts, `mcp-manifest.json` + its arch test, the load test, and the documentation. ⚠ **`notes` is in PR 1 deliberately** — it owns the private roots, so leak rows 7, 8, 10 and 11 are testable in the first PR rather than the last. ⚠ **The contract bumps in PR 1**, which is where the four token routes ship — *"bump `openapi.yaml` in the PR that earns it"*, on the checklist since v8. ⚠ **PR 1 is deployable and useless on purpose:** with no frontend the only way to mint a token is a hand-written INSERT on the droplet, and doing exactly that against a real client is how the credential gets exercised while the diff is still small enough to read. v10's D261 precedent, for the same reason: building it as one PR means the platform change is first written and last verified.
- **D322 — OAuth 2.1 is v12, explicitly.** A PAT cannot be a claude.ai remote connector: that path wants protected-resource metadata (RFC 9728), authorization-server metadata, dynamic client registration (RFC 7591), PKCE, refresh tokens and a consent screen — which would make Home an OAuth authorization server, or make `auth.tilcer.cz` one, and it speaks only Mode B internal mint today. Naming it as v12 is a decision, not a deferral: **v11 is a desktop surface** and its documentation says so wherever a reader might assume otherwise.

---

## V11-11. Acceptance Criteria (v11)

**The front door**

- [ ] `POST /mcp` completes `initialize` → `tools/list` → `tools/call` against a live token; `GET /mcp` returns **405** (D277).
- [ ] A request carrying a **valid session cookie and no bearer** returns **401** (D280).
- [ ] A request with an `Origin` outside `HOME_ALLOWED_ORIGINS` is refused; one with **no** `Origin` and a valid bearer succeeds (D281).
- [ ] `GET /mcp/nonsense` returns **JSON, not `index.html`** — asserted on the `Content-Type`, because the failure looks like a working server (D282).
- [ ] `tools/list` without a bearer is refused; `initialize` without one returns capabilities and **no tool names** (D312).
- [ ] `MCP-Protocol-Version` is echoed; an unsupported version is refused at `initialize` naming the supported one.

**The token**

- [ ] The secret appears in exactly one response in the whole contract and is **absent from `McpToken`** — asserted against `openapi.yaml`, not the handler (§V11-6).
- [ ] Only the SHA-256 is stored; a database dump contains no usable secret.
- [ ] `expires_at` is byte-identical before and after 50 tool calls (D285); `last_used_at` advances **at most once** across those 50 within an hour (D286).
- [ ] A revoked token 401s on the **very next call**, with no restart and no cache expiry (D287).
- [ ] With the owner's account closed in auth, the next call re-mints, fails closed and **stamps `revoked_at`**; the call after that is a clean 401 against a revoked token, **not a second re-mint** (D287).
- [ ] A `reader`'s token is refused every write tool and granted every read one (D287).
- [ ] `PATCH` cannot change `expires_at` — **422** (D285). An 11th live token is refused — **422** (D289).
- [ ] `DELETE` stamps `revoked_at` and **leaves the row**, so an audit event's `via_token_id` still resolves to a name (§V11-6).
- [ ] `GET /api/admin/mcp/tokens` contains **no secret and no hash**, and there is **no admin mint route anywhere in the contract** (D316).

**The catalog and the new host map**

- [ ] `internal/arch` stays green: no module imports another, and the MCP host imports no module (D276).
- [ ] `mcp_completeness_test.go` passes and **fails on a deliberately introduced duplicate tool name**, an unregistered module, an unreadable resource template, and a tool carrying `destructiveHint` (D319, D294).
- [ ] `backend/mcp-manifest.json` matches the live registry; **the test regenerates it and a stale file fails the build** (D320).
- [ ] The tool count is **≤ 45**, asserted (D293).
- [ ] Adding a module's provider requires **no change to `registry.Module`** — asserted against the diff (D276).

**The leak table — one test per row, written from the attacker's side**

- [ ] **Row 7:** `home_activity` run by member B over a window containing member A's private-note events returns the **redacted** phrase for A's `entity_type`, with the id blanked — asserted against a **second member's** item, not the caller's (D298).
- [ ] **Row 8:** `home_search` by B for a term occurring **only** in A's private note returns **no hits**; A gets one. Repeated per module that declares `Search` (D302).
- [ ] **Row 8 (the trap itself):** every provider given an **actor-less ctx** returns an error — not empty, not `AnyScope`. One test per provider (D302).
- [ ] **Row 11:** `resources/list` for B does not enumerate A's private roots or a conversation B is not in (D308).
- [ ] **Row 10:** `resources/read` on a URI B obtained out of band returns the **same** result as a URI that does not exist (D308).
- [ ] **Row 9:** every ownership- and membership-scoped tool returns a result **byte-identical** to the not-found result when the caller is refused — never the word *forbidden*, never a distinguishable code (D303).
- [ ] **Row 12:** reading a conversation writes exactly one `chat.read` event carrying the conversation id and a count, and **no body, snippet or message id** — asserted against `audit_events` and `audit_changes` (D297).
- [ ] **Row 12 (the invariant it must not break):** `TestChatMessagesAreNotAudited` **still passes** after v11 (D231, D296).
- [ ] **Row 14:** a forced panic inside a tool call produces a status report containing **no `Authorization` header and no token**, while still carrying the `stack` attr the crash board groups on (D313).
- [ ] **Row 15:** a resource read exceeding `HOME_MCP_MAX_RESULT_KB` is truncated by the **host**, with the truncation stated in the result (D307).

**Writes, and what does not exist**

- [ ] `tools/list` contains **no** delete, publish, purge, upload, membership, admin-mutation, season-close or layout tool — asserted against the manifest, by name (D295).
- [ ] ⚠ **A conversation cannot talk the server into a delete.** The criterion is the absence, not a refusal: a scripted attempt to call `home_notes_delete` returns *unknown tool* at the protocol level (D294, D295).
- [ ] A card created through MCP appears in an open browser **live**, and its Log row carries `via='mcp'`, `via_token_id`, and the label `"<display name> · <token name>"` (G2, D290, D291).
- [ ] The same write made in the browser carries `via IS NULL` — the filter distinguishes them, and the partial index is used (asserted on the query plan) (D292).
- [ ] **No write tool can reach a service with an input it could have refused** — one negative-number, one empty-string and one out-of-range case per writing module, each returning a 422-shaped result and never a 500 (D311).
- [ ] `POST /api/electricity/periods` with a negative `invoiced_vt_dkwh` **still returns 500**, and electricity's `limit` still falls back where the shared helper clamps — both defects recorded as unfixed, not silently repaired (D311).
- [ ] Every write tool's audit event is written **in the same transaction** as the change it describes — the house invariant, asserted for at least one tool per writing module.

**The pool**

- [ ] **Twenty concurrent tool calls from one token, and a browser request served throughout** — a load test, not a unit test (§V11-8).
- [ ] The 3rd concurrent call from one token waits rather than opening work (D304); the 121st call in a minute is refused (D306).
- [ ] A tool call exceeding the deadline is cancelled and **releases the connection** — asserted by a following query succeeding immediately (D305).
- [ ] No provider returns a `*sql.Rows` it has not closed; every list path goes through `appdb.Collect` — a review item on the whole diff, and the one thing `internal/arch` cannot check (row 17).

**Migrations**

- [ ] `02005` and `01003` apply cleanly on an empty database **and on a restored copy of production**, and both down-migrations run.
- [ ] ⚠ `01003` is verified **not** to rebuild `audit_events`: the migration contains no `CREATE TABLE`, no `DROP TABLE` and no rename, and `audit_changes` row count is identical before and after (D290).
- [ ] `idx_events_via` is a **partial** index and the Log's existing queries are unchanged in plan (D292).

**The frontend**

- [ ] The mint dialog reveals the secret once; reopening the token's row never shows it again; the dialog does not dismiss on outside-click or Escape (D315).
- [ ] The secret block scrolls **inside its own container** at 375 px and the page does not scroll horizontally (D315).
- [ ] Administrace's strip renders **three** level-1 groups and **eight** tabs without overflowing at 375 px (D314; ⚠ not D202's stale six).
- [ ] The axe sweep — green for the first time as of v10.2 — covers both new surfaces and stays green.
- [ ] ⚠ **A click-through with a second member's session**, which every live verification since v8 has been owed and none has had.

**Documentation, before the version is called done**

- [ ] The `#46` and `#47`/`#48` entries are written into the PRD and CHANGELOG, and Nextcloud is re-synced — **v11's first commit** (§V11-6).
- [ ] `backend/openapi.yaml` is bumped to **0.15.0 in the PR that earns it**, and the CHANGELOG carries an entry for it — the v7/v8 lesson, and the 0.14.0 gap, both closed.
- [ ] `HANDOFF-13-mcp.md`, the `HANDOFF-design.md` §v11 addendum, `README`'s env tables (⚠ including `HOME_CHAT_TRASH_DAYS`, still missing since v10) and `REGISTRY.md` all land in the same sitting as the code.

---

## V11-12. As built — the v11 build (2026-09-06/07, OpenAPI 0.14.0 → **0.15.0**, three PRs per D323)

> **Read this before trusting §V11-1…§V11-11.** Nothing below was declined and nothing was skipped; what changed is the shape of four things, and two defects older than v11 were fixed because measuring v11's own surfaces is what found them.
>
> `v11/mcp-front-door` (PR 1) · `v11/mcp-providers` (PR 2) · `v11/mcp-surfaces` (PR 3), each stacked on the last and each reviewed at `xhigh` until nothing medium-or-above stood.

### The catalog is 39 tools, and the count is now asserted rather than written down

**39 tools, 3 resource templates, 5 prompts**, which is what §V11-4 FR-M4 specified. The number is asserted in two places that cannot both be satisfied by a lie: `internal/platform/mcp`'s golden test builds the manifest **from the live registry over a real database with every real provider composed**, and `internal/arch/mcp_completeness_test.go` reads the committed file and checks it against the module directories, against `openapi.yaml`'s `McpModule` enum **in both directions**, and against D293's ceiling of 45.

⚠ **The tool catalog is the SEVENTH host map** on the PRD's own arithmetic (D213 counted six), and it is the first one that is generated rather than hand-maintained. `platform/storage`'s table declarations got the same treatment for a reason worth repeating: `StorageBlobs` was once implemented on the *Service* rather than the *Module*, everything compiled, every test passed, and the Úložiště page reported 0 B — *it was found by opening the page*. **There is no page to open here**, which is the whole argument for a committed artifact.

### Four things shipped in a different shape

- **`home_activity` resolves the caller's role before it answers**, which §V11-4's core-seven table does not say. The first review round found it reading the whole audit spine for a `reader`, a surface `/api/logs/**` has refused with 403 since D5. ⚠ **Read-only is not the same question as ungated**, and the version got that wrong twice: the same shape reappeared in PR 2 in `logging.Search`, where a `reader` would have received the household's audit summaries through `home_search`. That one was found by reading rather than by review.
- **`mcp.DecodeArgs` is STRICT**, which §V11-4 does not say either. It shipped lenient in PR 1 and a fourth round measured what that cost: `home_notes_create` with `body` instead of `body_md` created a note with an **empty body** and answered *"Poznámka vytvořena"*, while `POST /api/notes` with the identical payload is a 422 — `httpx.DecodeJSON` has set `DisallowUnknownFields` on all 178 handler call sites since v1. The second front door accepted what the first refuses.
- **The five prompts shipped EMPTY in PR 1 and were filled in PR 3.** Four of the five name tools that did not exist until PR 2, and a prompt telling a model to call a tool the server does not publish is worse than no prompt: the model follows it, gets an unknown-tool error, and reports the server as broken. ⚠ And one prompt does not say what the handoff said. `HANDOFF-13` §10 describes `/večeře-z-toho-co-máme` as `garden.storage` + `garden.harvests`; its own tool table four sections earlier publishes neither as a read — garden is six tools and the sklad is not among them, while `harvest_log` *writes* a harvest rather than reading them back. The body reaches for `home_garden_plan` and `home_garden_tasks` and says in one visible line that the sklad is not readable from here — in the **body**, where the model reads it, because a model that does not know a thing is missing invents it.
- **The load test is three tests.** §V11-8 asks for one. The concurrent test proves a leak exists and cannot say whose; a sequential sweep calls each read tool alone and then asks the browser a question, so the first tool after which the browser stops answering is the culprit; and a third leaks a `*sql.Rows` **deliberately** and asserts the browser stops being served, then closes it and asserts the browser comes back. Without the third, the other two rest on a premise nothing checks — and would go on passing if the pool were ever widened to two connections. Measured: 80 calls, 20 concurrent, ~37 ms, 24–28 browser requests served throughout, all overlapping, slowest 1 ms.

### Two contrast defects, both older than v11

The design bundle ships exactly one token change for this version, and the build ships one more that measuring forced.

- **`--subtle`** — light **0.580 → 0.515**, dark **0.660 → 0.700**. Open since v10.2 (§V10-15 recorded five per-usage swaps and said outright that the pairing was a token decision for the design bundle rather than an implementation PR's to take). These two screens forced it: prefix, dates, scope and *naposledy použito* are all secondary text, which makes Asistenti the worst possible place to inherit a metadata colour that does not measure. Light now s1 5.60 · s2 5.26 · s3 4.91 · s4 4.56.
- **`--accent` on `--accent-soft`** — **3.77:1** dark and **4.33:1** light at 12.5 px, on the selected state of **eight** controls including the app's side nav. Found by the axe sweep on the Asistenti tab strip the moment there was a sweep that visited `/administrace`. Fixed in the **token** (soft 0.360 → 0.300 dark, 0.930 → 0.950 light; 4.74 and 4.57 now) rather than at eight call sites — which is precisely what NastaveniPage's own v10.2 note asked for, and which keeps a version promising *no new colour, one corrected value* from restyling the primary navigation.

⚠ **A third was found and deliberately NOT fixed.** `--warn` on `--s1` measures **4.07:1** in the light theme at 12 px, on Administrace → Rozeslat. It is a serious AA failure, it is not v11's surface, and this version repairs nothing on its way past — the same discipline that made a directory-wide `gofmt` get reverted out of PR 2. `/administrace` is therefore left out of the page-level axe path list with the measurement written above it, and the Asistenti tab carries its own sweep instead.

### What the review rounds cost, and what they were worth

Twelve rounds across three PRs, every one in a fresh subagent. PR 1 took five: a **Critical** (`home_activity` above), two **Highs** (an unresolvable bearer ran a query on the one connection with nothing bounding it; the rate limiter's map never evicted `X-Forwarded-For`-derived keys, so one flood with a rotating header was one permanent entry per request until the container was OOM-killed), then four Mediums, then three budget-arithmetic Mediums, then the lenient decode. PR 2 took four, and the class is named in the CHANGELOG. PR 3 took its own, written up two sections below.

⚠ **Two findings are worth carrying forward as classes rather than as incidents.**

**A published schema that disagrees with the service beneath it** is invisible to every test that calls the service directly, and a model following the schema exactly gets a 422 every time with nothing to correct. It appeared in three consecutive PR 2 rounds. What finally swept it was not a better reading: it was *calling* all ten write tools with minimal schema-conformant payloads and comparing every published enum value-for-value against the enum the service enforces.

**A guard that cannot fail is not a guard.** The reveal dialog's three `preventDefault` handlers refused Escape and an outside click against a `<Dialog.Root open>` with no `onOpenChange` — which stays open regardless. The test written to prove Escape does not dismiss **passed with every guard deleted**, and it was found by deleting them on purpose. `onOpenChange` is now wired the way every other dialog wires it, so the refusals are load-bearing. Same shape as v9's storage catalog: a surface asserted and a behaviour never called.

⚠ **And the same shape survived one layer down until PR 3's own review found it.** With `onOpenChange` wired, *three* handlers still meant *two* guards: Radix fires `onPointerDownOutside` **and** `onInteractOutside` for one pointer-down and dismisses only if neither prevented it, so each covered for the other and deleting either alone reddened nothing — while this paragraph claimed every deletion was caught. `onPointerDownOutside` is gone; `onInteractOutside` stays because it is the wider of the two (it fires for a focus move outside as well), and each of the two remaining deletions was verified to redden a named test by making it.

### The review round PR 3 took

One **Medium** and eleven low or cosmetic. The Medium is the class this version keeps naming: **`prompts/get` read the prompt's `name` and never its `arguments`**, so `měsíc` — declared in `prompts/list`, published in `mcp-manifest.json`, collected by the client from the member — was discarded, and `/měsíční-uzávěrka` for August answered about July with nothing anywhere saying so. It is the silently-dropped value that `DecodeArgs`, the `modules` allowlist and the `via` filter each refuse, arriving on the one surface where there is no error to see. Arguments are now substituted into a `{{name}}` placeholder, an undeclared name is a protocol error rather than a shrug, a `Required` argument that is missing is refused, and a golden test asserts that **every** declared argument appears in its own body — the guard against the class rather than against the incident. The rest: both token surfaces rendered their empty state while the query was still pending, so a member with three tokens read *"Zatím nemáte žádný token"*; the admin owner filter could stick with no chip left to clear it; `člena ${owner}` put a nominative display name after a genitive noun; the two load tests counted an `isError` result as a call that ran, which would have made the leak sweep vacuous for any provider that started refusing; `compactSchema` re-escaped `<` and `&` and undid `MarshalManifest`'s deliberate `SetEscapeHTML(false)`; and four comments measured or described something other than what the code does — including this file's own `--subtle` note, which quoted 4.05 against the `--accent-soft` the hunk below it was changing.

### What is outstanding

- **The `/mcp` Coolify path route** (D318) is the one deploy step nothing in the repo can do. Without it the request never reaches the binary; it lands on the frontend's Nginx and gets `index.html` **with a 200**.
- **`REGISTRY.md` is not updated** — it lives in Nextcloud and cannot be reached from this repo. Same gap as the `#46`/`#47`/`#48` masters, which is why those entries were reconstructed from merged diffs as v11's first commit.
- **A click-through with a second member's session**, owed since v8 and still owed. Every backend leak-table row is asserted from the attacker's side against a real database, and the frontend e2e runs as one member under the dev bypass.
- **`--warn` on `--s1`**, above.
