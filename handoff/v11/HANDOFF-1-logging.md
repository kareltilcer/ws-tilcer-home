...
> **v2:** packaged as a module per `HANDOFF.md` §3 (own routes/migrations/audit actions, registered via the core; no cross-module imports). Auth is Mode B — authorization is from the home **session**, not a JWT; the role gate (log browser = `admin`) is unchanged. The spine's `AuditSink` lives in `platform/` (so every module can call it) while its **tables** are this module's migrations. Behaviour below is unchanged from v1.
