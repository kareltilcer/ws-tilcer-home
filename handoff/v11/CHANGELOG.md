...
## v11 — 2026-09-06 (spec) · **PR 1 of 3 merged**, PRs 2 and 3 outstanding · The MCP front door (`platform/mcp`)
...
**Still outstanding after PR 1:** the remaining six providers, `logging`'s search-only one, the cross-module search's full fan-out, resources for documents and chat, and the `chat.read` audit action (PR 2); the two frontend surfaces, the five Czech prompts, `backend/mcp-manifest.json` with its `internal/arch` completeness test, and the load test (PR 3). ⚠ **PR 1 is deployable and useless on purpose**: with no frontend the only way to mint a token is a hand-written INSERT on the droplet, which is exactly how the credential gets exercised against a real client while the diff is still readable (D323).
