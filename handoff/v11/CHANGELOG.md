...
## v11 — 2026-09-06 (spec) · **all three PRs built** · The MCP front door (`platform/mcp`)

> **OpenAPI → 0.15.0**, and it ships in **PR 1 because PR 1 is where the four token routes are** — *bump the contract in the pull request that earns it*, on the checklist since v8, and the reason the 0.14.0 gap below had to be written up first. Two migrations, **no new module and no new migration block**: `02005_mcp_tokens` (platform) and `01003_audit_via` (logging). Decisions **D275–D323** (`PRD.md` §V11-10), acceptance criteria in **§V11-11**, build guide `HANDOFF-13-mcp.md`. Three pull requests by D323, and this entry grows with each.

**v11 adds no twelfth module. It adds a second front door to all eleven.** `POST /mcp` speaks JSON-RPC 2.0 over Streamable HTTP, mounted **outside** the `/api` group with its own chain — bearer, Origin allowlist, rate limit, a semaphore of two per token, a per-call deadline (D279) — and authenticated by the first bearer credential Home has ever accepted for its own data: a personal access token, `hmcp_` + base64url(32 bytes), shown once, stored only as SHA-256, expiring on a schedule its owner picks and **never sliding** (D284/D285). The session cookie is not read there at all, and the bearer is refused everywhere else (D280).

**Modules publish through the fifth registered catalog** — an optional `mcp.Source`, type-asserted at composition exactly as `metrics` and `lists` are, and deliberately **not** part of `registry.Module` (D276/D56). **PR 1 ships three providers — `todo`, `events`, `notes`** — beside the seven cross-cutting tools. ⚠ `notes` is in the first PR on purpose: it owns v9's private roots, so the leak table's rows 7, 8, 10 and 11 are testable while the diff is still small enough to read.

**Every write through a token is attributed and marked.** `audit_events` gains `via` and `via_token_id` by `ALTER TABLE ADD COLUMN`, read from the context by the sink so no handler can forge them and none of the 28 `Record` call sites changed. ⚠ **Not a fourth `actor_type`, and that is a decision rather than a shortcut** (D290, confirmed by Karel): SQLite cannot ALTER a CHECK, so widening the enum means rebuilding the parent of `audit_changes` — an implicit DELETE that fires the cascade and destroys the household's entire change history **while reporting success**. `TestActorTypeStaysThree` is what keeps that found rather than repaired.

**The shape of the surface is decided by what is ABSENT** (D294/D295). Delete, publish, purge, upload, membership change, admin mutation, season close and dashboard layout are not gated and not confirmed — they do not exist, so the model cannot propose them, and no tool carries `destructiveHint`. What a token DOES see is everything its owner sees, private roots included, so the whole risk of the version is **§V11-4's seventeen-row leak table**, which ships here as one test per row written from the attacker's side. ⚠ **Read-only is not the same question as ungated**: `home_activity` reads the audit spine, whose routes have been admin-only since D5, so it is the one tool in the core seven that resolves the caller's role before it answers.

⚠ **`/mcp` needs a new Coolify path route to the backend on 7999** — its third, after `/api` and `/ws`, Strip Prefix **off** (D318). Without it the request never reaches the binary: it lands on the frontend's Nginx, which answers `index.html` **with a 200**, and the client reports a protocol error against a perfectly healthy server. The Go router has an identical-looking bug it now guards against (`/mcp` joined `/ws` in the SPA exclusion, D282) — same symptom, different layer, so the README says to check both and in which order.

**Six new environment variables**, all `HOME_MCP_*`, all defaulted, **none secret**, and all refused at both ends rather than clamped — nothing is upgrading into them, so an out-of-range value can only be a fresh mistake (§V11-9). `HOME_MCP_ENABLED` defaults **on** (D317) and says so once on the boot line, because a disabled server and a misconfigured client look identical from the client.

⚠ **Two review rounds ran against PR 1, and the second one found four things the first did not.** A tool taking two ids checked neither against the other, so `home_todo_checklist` ticked a box on a card the caller had not named and answered with a checklist the change was not in. `resources/list` spent the per-module SEARCH budget as a listing budget, showing ten notes out of forty with nothing marking the cut. `home_search`'s `in` filter swallowed a module name that does not exist and answered *nothing found* — the failure the token allowlist's own 422 exists to prevent, in the surface next to it. And the Log's `via` filter was the one filter on that route built as a switch rather than an equality bind, so a value outside the contract's `enum [mcp, ui]` added no condition at all and returned the whole spine as though nothing had been filtered. All four are now refusals, each with a test written from the side that gets it wrong. Three smaller shapes went with them: an unsupported `MCP-Protocol-Version` header is a **400** naming the one home speaks rather than being echoed back as though home spoke it too; a PATCH that changes nothing writes no `mcp.token.update`; and a store failure resolving a real metric or list key no longer reports itself as an unknown key, which was D311's retry loop reached through the wrong text instead of the wrong status.

⚠ **A third round found the same defect in two more budgets, and one false alarm worth recording.** `home_search`’s `in` filter validated a TRIMMED module name and then matched on the RAW one, so `"notes "` walked past the refusal round 2 had just added and into a set no provider’s `Module()` can equal — nothing ran, no per-module count came back, and the answer was *0 hits* from a household with plenty: the silence that refusal exists to prevent, reached through it rather than around it. The Log’s `via` filter, written in the same commit, refuses `"mcp "` outright, and only one of the two spellings can be right. Two caps were then found being spent twice rather than once: `notes.ListResources` took the host’s whole listing budget from EACH of its two roots — up to 400 rows against a cap of 200, and the host’s “this page was cut” warning firing on pages nothing had cut — and `resultWire` measured the text and the structured payload against `HOME_MCP_MAX_RESULT_KB` separately, so a tool answering with a capful of each shipped twice the figure whose whole purpose is to bound what reaches the model’s context. The private root is now enumerated first, so what a shrinking budget drops is the shared tree rather than the half only its owner can see; a dropped structured half now says so, the way the truncation note and the oversized-blob refusal already do. Two smaller ones went with them: an oversized blob was measured RAW and shipped base64, which is four bytes out for every three in, so a file a third over the cap went through and reported a size the reader could not reconcile with what arrived; and the host’s own seven tools decoded their arguments three different ways while `mcp.DecodeArgs` — extracted the round before precisely so D311’s rule lives in one place — was used only by the three providers. ⚠ **The false alarm is the one worth keeping**: the round reported that a disabled `/mcp` answers chi’s built-in `text/plain` 404, because chi’s `Mount()` copies a parent’s NotFound onto a subrouter only if the parent has one at mount time and `httpx.NewRouter` registers its own afterwards. It does not — `Mux.NotFound` also walks the subroutes already mounted — and the finding was DECLINED after a test proved it. `TestDisabledServerIs404` now asserts the Content-Type rather than merely “not HTML”, so the behaviour is held by a test instead of by a reading of chi.

⚠ **A fourth round found one Medium, and it is the same rule this version keeps applying and then missing by one surface.** `mcp.DecodeArgs` decoded a tool call's arguments leniently, so a field outside the tool's declared `inputSchema` — every one of which says `"additionalProperties": false` — was silently DROPPED rather than refused. `home_notes_create` with `body` instead of `body_md` created a note with an **empty body** and answered *"Poznámka „Probe“ vytvořena"*; `POST /api/notes` with the identical body is a 422, because `httpx.DecodeJSON` has set `DisallowUnknownFields` on all 178 handler call sites since v1. `home_notes_tree` with `noteId` instead of `note` answered with the **whole shared tree** rather than the one note asked for. Both read to a model as success, and both are the silently-ignored value this version refuses everywhere else — an unknown module is a 422, an unknown `via` is a 422, an expiry on PATCH is a 422 rather than a no-op. The decode is now strict and names the offending field the way its HTTP twin does. Four smaller ones went with it: `MCPAuth.identity` answered "no authenticator, no bypass actor and no cached session row" with `ErrUserClosed`, which `Resolve` turns into an irreversible token revoke — the one classification `classifyMint` exists to keep separate, now `ErrUnreachable`; `mcp.Hit` carried no JSON tags, so `home_search` was the only tool in the catalog whose `structuredContent` came back in Go field names; `home_events_create` let `reminder_enabled: true` with no lead reach the service and answer in the service's English, against D311's rule stated in that same file; and the truncation notes were appended AFTER the cap rather than paid for out of it, so the one result that announced its own truncation was the one result that exceeded the figure. Two documentation corrections closed the round: this file said *PR 1 of 3 merged* while `PRD.md` and `HANDOFF.md` still said **NOT BUILT**, and `TestDisabledServerIs404`'s comment narrated round 3's DECLINED false alarm as though it had been a real defect — which is exactly how a declined decision gets repaired by the next reader.

**PR 2 added the other seven providers, and found one class of defect four times.** `documents`, `finance`, `garden`, `electricity`, `chat`, `admin` and `logging`'s search-only one, plus the cross-module search's full fan-out, `documents` and `chat` resources, and `chat.read` (D297). ⚠ **The class worth naming is a provider's published JSON Schema disagreeing with the service beneath it**, because a model that follows the schema exactly then gets a 422 every time and cannot see why. It appeared in three consecutive rounds — `kind` published as a free string against a closed enum; `rates` optional against a service that requires them; `area_m2`/`plant_count` both optional at `minimum: 0` against a `validatePlantingShape` that demands exactly one, strictly positive, so `home_garden_planting_create` **could not be called at all**; `unit` and `destination` published as free strings against enums matched by exact code. Round 3 swept it by *calling* all ten write tools with minimal schema-conformant payloads and comparing every published enum value-for-value against the one the service enforces; round 4 came back clean. Two reads answering the wrong row went with them, and one defect the review did not find: **`logging.Search` would have handed a `reader` the household's audit summaries through `home_search`** while their browser answers 403 on `/api/logs/**` — the same shape as PR 1's Critical, second occurrence, now an admin gate returning nothing with a control assertion proving an admin does get hits.

**PR 3 is the half a member ever sees, and one file that is a contract.** Nastavení gains **Asistenti (MCP)** as a fourth section and Administrace a **third level-1 group of one tab** — eight tabs now, and its own header comment had said *four* since v5. The Log gains an eighth filtering dimension (*Vše · V aplikaci · Přes asistenta*) and a neutral chip on rows written through a token: neutral because an assistant's write is an ordinary change with a known origin and will be the Log's most common future row, and nameless because the actor beside it already reads `Karel · Claude (notebook)` (D291). Five Czech prompts ship filled in — they were deliberately empty in PR 1, because four of the five name tools that did not exist until PR 2 and a prompt naming a tool the server does not publish is worse than no prompt. And **`backend/mcp-manifest.json`** is generated from the live registry and committed: 39 tools, 3 resource templates, 5 prompts, 43 kB. ⚠ **openapi cannot describe the MCP surface** — it is JSON-RPC with a JSON Schema per tool (D320) — so the contract is a golden file diffed in review, with `internal/arch/mcp_completeness_test.go` checking it against the module directories, against openapi's `McpModule` enum in **both** directions, and against the verbs v11 publishes nothing for.

⚠ **THE ACCEPTANCE CRITERION WAS A LOAD TEST, AND IT IS THREE TESTS RATHER THAN ONE.** *"Twenty concurrent tool calls from one token, and a browser request served throughout"* is now twenty goroutines released from one barrier, four calls each, spanning **every read tool in the catalog** rather than one tool twenty times — because leak row 17 is a property of a *provider*, and twenty copies of `home_whoami` prove nothing about the other eighteen. 80 calls in ~37 ms, peak 20 in flight, 24–28 browser requests served throughout, every one of them overlapping, slowest 1 ms. A sequential sweep is what *names* the culprit if it ever fires — twenty goroutines wedged on one connection produce a stack dump in which everyone is waiting and nobody is the cause. And a third test leaks a `*sql.Rows` **on purpose** and asserts the browser stops being served, then closes it and asserts the browser comes back: without it the other two rest on a premise nothing checks, and would go on passing if the pool were ever widened.

⚠ **PR 3's own review round found one Medium, and it is this version's signature defect on the one surface that cannot report it.** `prompts/get` read the prompt's `name` and never its `arguments`, so `měsíc` — declared in `prompts/list`, published in `mcp-manifest.json`, and collected from the member by their client — was **discarded**: `/měsíční-uzávěrka` for August answered about July, plausibly, with nothing on the wire saying anything had been dropped. That is the silently-ignored value `DecodeArgs`, the `modules` allowlist and the `via` filter each refuse with a 422, arriving where there is no error to refuse with. Arguments are now substituted into a `{{name}}` placeholder, an undeclared name is a protocol error, a missing `Required` one is refused, and a golden test asserts **every** declared argument appears in its own body. Eleven low or cosmetic went with it: both token surfaces rendered their empty state while still loading, so a member with three tokens read *"Zatím nemáte žádný token"*; the admin owner filter could stick with no chip left to clear it; `člena ${owner}` put a nominative name after a genitive noun; the two load tests counted an `isError` result as a call that ran; `compactSchema` re-escaped `<` and `&`, undoing `MarshalManifest`'s deliberate `SetEscapeHTML(false)`; and four comments described something other than what the code does.

⚠ **The reveal dialog's guards were decorative, and it took deleting them to find out.** `<Dialog.Root open>` with no `onOpenChange` stays open whatever happens, so the three `preventDefault` handlers that refuse Escape and an outside click were doing nothing — and the test written to prove Escape does not dismiss passed with every one of them removed. `onOpenChange` is now wired the way every other dialog in Home wires it, which makes the refusals load-bearing. ⚠ **And the same shape survived one layer down until the round above.** Three handlers still meant two guards: Radix fires `onPointerDownOutside` **and** `onInteractOutside` for one pointer-down and dismisses only if neither prevented it, so each covered for the other and deleting either alone reddened nothing. `onPointerDownOutside` is gone, `onInteractOutside` stays because it is the wider of the two, and each remaining deletion was verified to redden a named test by making it. It is the same shape as v9's storage catalog: *a surface asserted and a behaviour never called*.

⚠ **Two contrast defects, both older than v11, both found by measuring rather than by looking.** `--subtle` is the one token the design bundle ships for this version — light **0.580 → 0.515** (s1 4.29 → 5.60, s2 4.04 → 5.26, s3 3.76 → 4.91), dark **0.660 → 0.700** — open since v10.2 and forced by these screens, where prefix, dates, scope and *naposledy použito* are all secondary text. And `--accent` on `--accent-soft` measured **3.77:1** dark and **4.33:1** light at 12.5 px, on the selected state of eight controls including the side nav; the axe sweep found it on the Asistenti tab strip the moment there was a sweep that visited it. Fixed in the **token** (soft 0.360 → 0.300 dark, 0.930 → 0.950 light; now 4.74 and 4.57) rather than at eight call sites, which is what NastaveniPage's own v10.2 note had asked for. ⚠ **`--warn` on `--s1` measures 4.07:1 in the light theme** on Administrace → Rozeslat and is **not** fixed here: that tab is not a surface v11 touches, and this version repairs nothing on its way past. The measurement and the reason are written into the axe spec above the path list.

**Deploy needs one thing Coolify does not do by itself:** the `/mcp` path route to the backend on 7999, Strip Prefix off (D318). Without it the request lands on the frontend's Nginx, gets `index.html` **with a 200**, and the client reports a protocol error against a healthy server.

---

## v10.2 — 2026-09-05 · The shell's own chrome (`app shell` · `settings`)

> **No API change** — OpenAPI stays at **0.14.0** and no migration ships. (0.13.0 was v10.1; `backend/openapi.yaml` has carried **0.14.0** since the same-day reminder lead `"0d"` was added to the event surfaces — the entry for that pass is below, written six weeks late.) Decisions **D271–D274** (`PRD.md` §V10-10), as-built record in **§V10-15**. From the `design/v10_2` bundle plus three requests from Karel, all of them about the frame around the modules rather than about any module.
...
**The deployed version is visible** (D271), in the two places the artboards put it — the foot of the side nav and the bottom of the *Více* sheet, where a hint describing the sheet to somebody already looking at it used to be. The label reads **`v1.10.2 · a3f9c2e`**; the commit is a build arg defaulting to Coolify's `SOURCE_COMMIT`. A value that is not a sha is dropped rather than printed.

⚠ **The version half is `frontend/package.json`, not a constant in `version.ts`** (D274). It was a string literal for the first cut of this release, and `package.json` had carried Vite's scaffold default `0.0.0` since the repo was created — two files claiming to say which release this is, which had never once agreed. The manifest is what a tool reads and the constant was what a member copies into a bug report, so neither end could see the other was wrong. ⚠ **The label is therefore NOT this file's number**, and the difference is a leading `1.`: npm demands three-part semver while the releases here are numbered `v10`, `v10.1`, `v10.2`, so the release is carried in `minor.patch` and the major is a constant `1` that means nothing. `1.10.2` is v10.2, `1.11.0` will be v11, and `version.test.ts` asserts that mapping against the newest heading above rather than leaving it as prose.

⚠ **Coolify excludes `SOURCE_COMMIT` from the build by default** — *Include Source Commit in Build* is the toggle, and without it the label reads `v1.10.2` alone, which still names the release. **The cache reason for leaving it off does not apply to `frontend/Dockerfile`**: the layer under the arg is `RUN npm run build`, while `RUN npm ci` sits above `COPY . .` and is keyed only on the two manifest files, which the arg cannot reach — so the toggle never re-runs the install. (A *release* commit does, because D274 put the version in `package.json`; that cost is the release's, not the toggle's.) The only builds that pay for the toggle are commits touching nothing under `frontend/` — on any commit that touches something, `COPY . .` has already invalidated the build layer.

⚠ **AND THE TOGGLE ALONE WAS NOT ENOUGH.** The first deploy with *Include Source Commit in Build* on, cache cleared, still shipped a commit-less label — `VITE_APP_COMMIT` baked as `""` in `dist/assets/*.js` while every other `VITE_*` value arrived intact. The defect is one sentence: **a default written in the Dockerfile beats the `ARG SOURCE_COMMIT=<sha>` line Coolify splices in** — in-stage it resets the injected value, above the `FROM` it shadows it — and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, so `version.test.ts` asserts it, and its position, rather than leaving either to the comment above it. Every supply path was built against the real Dockerfile and read back out of `dist/assets/*.js` — splice above the `FROM`, splice after it, `--build-arg`, a key-only `--build-arg` with the name unset, and nothing at all — and bare is the only spelling right under all of them. ⚠ **One value defeats bare too, and it is the only one: an explicitly empty one** (`--build-arg SOURCE_COMMIT=`, or the key-only flag with the name exported empty), which is why no Coolify variable named `SOURCE_COMMIT` or `VITE_APP_COMMIT` may exist. ⚠ **What is NOT established is why the deploy fell through to the splice** — had the flag path carried a value, `=""` would have lost and the label would have been right, so something kept `SOURCE_COMMIT` out of the build's environment. That answer lives in Coolify, not here. The bare `ARG` is correct either way; the README carries the measured table and the order to check things in.

⚠ **Five contrast fixes, and the e2e sweep found three of them.** Adding `/nastaveni` to the axe run turned up two serious violations on that screen — and then one on the sidebar's `admin` tag that had been failing the suite at light/1440 **for releases**, unnoticed because the suite had never been run green. The other two came from reading rather than running, on surfaces the sweep does not reach: the same `admin` tag in the mobile "Více" sheet, and the push-prompt's *dismissed* line, which only renders in a state no e2e run enters. `--subtle` on `--s1` measures **4.29:1** in the light theme, under the 4.5:1 AA bar; `--muted` is 6.81:1. Same swap `theme/globals.css` already made once for the foreign bubble's author label. The pairing is wider than these five usages and belongs to the design bundle's tokens, not to a per-usage fix — but the suite is green for the first time, which closes an item that has been outstanding since v5.

---

## Between v10.1 and v10.2 — 2026-09-03/04 · Two passes that shipped with no entry (`events` · `platform/statusreport`)

> **This heading is not a release, and that is the record rather than a tidy-up.** Two pull requests merged between v10.1 and v10.2 and neither was written down anywhere — not here, not in `PRD.md`. One of them **moved `backend/openapi.yaml` to 0.14.0**, which is why the v10.2 entry above cites a contract version this file had never named. Written retroactively on 2026-09-06 as v11's first commit, reconstructed from the merged diffs, so that 0.15.0's entry does not stack on a gap. As-built records: `PRD.md` **§V10-16** and **§V10-17**.

**A reminder for the day itself** ([#46](https://github.com/kareltilcer/ws-tilcer-home/pull/46), OpenAPI **0.13.0 → 0.14.0**, migration `04002`). Okno offered *Připomenout **předem*** and five lead times, the shortest a whole day — so an event you want to be reminded about **on** the day had to be entered as a reminder for yesterday, or with no reminder at all. The checkbox now reads *Připomenout* and the selector opens with **v den události**, a `reminder_lead` of `0d`.

⚠ **The lead arithmetic needed nothing and the database needed a table rebuild.** `subtractLead` returns `occurrence − lead` and `0d` is the identity; `reminder_lead` is `CHECK`-constrained and SQLite cannot alter a CHECK. ⚠ **`events` is the first rebuilt table here with INCOMING foreign keys** — `event_links` and `event_reminder_completions` both `REFERENCE events (id) ON DELETE CASCADE`, so with `foreign_keys=ON` a plain five-step rebuild would `DROP TABLE events`, fire both cascades, and take every link and the household's entire completion history **while reporting success**. `04002` renames the parent instead: a rename rewrites every REFERENCES clause naming the table, both children are rebuilt to point back at the new `events`, and only then is `events_old` dropped. No row is deleted anywhere in the file. The **down** migration remaps `0d` → `1d` *before* it narrows the CHECK, because the narrow CHECK would otherwise reject `0d` on the copy step and fail halfway.

All five schemas naming a lead took the new member (`Event`, `EventCreate`, `EventUpdate`, `Occurrence`, `DashboardReminder`) — minor rather than patch, because a client validating against a pinned 0.13.x copy would reject an event the household creates with it.

**Crash reporting and a feedback door** ([#47](https://github.com/kareltilcer/ws-tilcer-home/pull/47), [#48](https://github.com/kareltilcer/ws-tilcer-home/pull/48), **no API change**). `home` reports to **status.tilcer.cz** — errors and crashes from both halves, plus a way for the household to write to Karel from inside the app. Four `STATUS_*` variables, deliberately **not** `HOME_`-prefixed because they are the fleet-wide names every `ws-tilcer-*` service reads.

⚠ **The backend seam is the LOGGER, not a call beside each failure.** `platform/statusreport.NewLogHandler` wraps the slog handler in `main` and forwards every record at `slog.LevelError`, so all 54 existing `logger.Error(...)` sites report with none of them edited — which works only because the curation already existed: this service reserves `Error` for a fault that should not happen and logs everything soft at `Warn`. ⚠ **And it is where `Report.Context`'s "not user content" rule actually bites**, at the `logger.Error` call rather than in the handler: an Error line's attrs travel to a board read by Karel's admin session, which is a different lock from the one on a member's private note.

Two events are `fatal`, both written by hand in `main` — a panic that unwinds it, and a boot that returns an error, which is the crash-loop a failed migration produces. Everything the forwarder sends is `error`: a panic an HTTP handler recovers from killed the request, not the process. The feedback trigger is home's own control (sidebar footer, last row of the mobile *Více* sheet) because the widget's floating launcher sits 16 px from a corner and does not know about home's 56 px thumb-tab bar. Every path fails safe: an ingest error is dropped in silence, `Capture` never blocks a request path, and both halves honour a 429 by muting and **dropping** rather than queueing. ⚠ **The price is that a wrong key looks exactly like a quiet week**, which is why the boot line says which state reporting is in, once.

⚠ **The dialog's own Send button is unreachable on a phone and cannot be fixed from this repo** — that dialog is the status widget's, inside a closed shadow root with `all:initial` on its host. It reaches home only when status's frontend is rebuilt.
