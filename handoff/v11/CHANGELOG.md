...
⚠ **The reveal dialog's guards were decorative, and it took deleting them to find out.** `<Dialog.Root open>` with no `onOpenChange` stays open whatever happens, so the three `preventDefault` handlers that refuse Escape and an outside click were doing nothing — and the test written to prove Escape does not dismiss passed with every one of them removed. `onOpenChange` is now wired the way every other dialog in Home wires it, which makes the refusals load-bearing; deleting any of them now reddens a named test, verified by doing it. It is the same shape as v9's storage catalog: *a surface asserted and a behaviour never called*.
