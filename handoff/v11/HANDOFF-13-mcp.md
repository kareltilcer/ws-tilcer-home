...
⚠ **And `mcp.DecodeArgs` is STRICT — PR 2's six providers inherit that and must not decode around it.** Every `InputSchema` declares `"additionalProperties": false` and `httpx.DecodeJSON` has refused an unknown field on all 178 HTTP handler call sites since v1, so a lenient decode made the SECOND front door to the same eleven modules accept what the FIRST one refuses. PR 1 shipped it lenient and a fourth review round measured it: `home_notes_create` with `body` instead of `body_md` created a note with an **empty body** and answered *"Poznámka vytvořena"*, while `POST /api/notes` with the identical body is a 422; `home_notes_tree` with `noteId` instead of `note` answered with the **whole shared tree** instead of the one note. Both read to a model as success. It is the same silently-ignored value this version refuses everywhere else — an unknown module is a 422, an unknown `via` is a 422, an expiry on PATCH is a 422 rather than a no-op — applied at last to the arguments themselves. `TestUnknownToolArgumentIsRefused` is what goes red.

⚠ **A validator that needs the STORED row is the service's, not the tool's, and `events` is the line between them.** `home_events_create` can decide "a reminder switched on with no lead" alone, because there is no stored lead to merge with — and it must, or the refusal comes back in the service's English on a surface whose every other refusal is Czech. `home_events_update` cannot: `UpdateEvent` validates the MERGED state, so an event that already carries a lead may legitimately have its reminder switched on with no lead in the patch, and pre-validating there would refuse a correct call. The tool keeps only the half it can answer without a read.
...
- ⚠ **And a provider that reads TWO roots spends ONE budget across both, not `limit` from each.** `notes` did the second and returned up to 400 rows against a cap of 200 — a bound whose only job is to protect the one connection, doubled, and the host’s “this page was cut” Warn firing on pages nothing had cut, because two roots totalling the cap look exactly like one root that filled it. It is the same arithmetic `Search` gets right (D301). ⚠ **The caller’s own private root is enumerated FIRST**, so what falls off the end when a household outgrows the cap is the shared tree — still reachable through `home_search` and `home_notes_tree`, and announced by that Warn — rather than the half only its owner can see, which would vanish looking exactly like a member who keeps no private notes. `documents` mirrors this shape in PR 2 and inherits the rule; `TestNotesResourceListingSpendsOneBudgetAcrossBothRoots` is what goes red.
