...
>
> ⚠ **This index went stale between v9 and v11 and the correction is recorded rather than quietly applied.** It described a five-module app in its own title, carried no v10 entry at all, and its build-guide table stopped at `HANDOFF-11`. v10 (Chat), v10.1, v10.2 and `HANDOFF-12-chat.md` were all missing. They are added below. ⚠ **Two shipped passes still have no PRD or CHANGELOG entry anywhere** — `#46`'s same-day reminder lead (which moved the contract to **0.14.0 with no changelog entry**) and `#47`/`#48`'s status.tilcer.cz integration, whose only written record is the README. Writing those is **v11's first commit**.
>
