# Home — Implementation Handoff (Claude Code) — v2 *(foundation; live app = **v10.2**, eleven modules; **v11 spec complete, not built**)*
...
>
> **v9 (spec 2026-08-21 — BUILT AND DEPLOYED 2026-08-25, PRs #21 `bcd3c2f` + #22 `a7d7e17`):** the first version of Home that adds **no module**. It changes three that exist — `notes`, `documents`, `admin` — and adds one platform strand, **`platform/storage`**. Build guide `HANDOFF-11-privacy-storage.md`, scope brief `V9-privacy-storage-brief.md`, `PRD.md` §V9-1…§V9-11 + `openapi.yaml` **0.11.0**, decisions **D176–D215**, design brief `HANDOFF-design.md` **§v9**. Poznámky and Dokumenty each gain a **second root** — the uploader's own, unreadable by anyone else **including admins** (404, never 403) — publishing is one-way and owner-only, audit events are redacted at read time, and Administrace gains an **Úložiště** snapshot plus a **Soukromé položky** purge screen. ⚠ **Do not build it like `HANDOFF-5`…`HANDOFF-10`.** Those were additive: a new package, a new migration block, a registry entry, four host maps. v9 has **no new module and no new migration block**; it alters four tables carrying real data since v3/v4 and invalidates an assumption ~40 call sites were written against — *until v9, every row in `notes` and `documents` was visible to every member*. **`PRD.md` §V9-4a — the leak table — is the build checklist and the test plan**, twenty-three surfaces, twenty-two deny and one grants — and it had eighteen until a review pass found three real holes in it (D206–D213), so read its length as a floor. New ops prerequisites: **none** — two defaulted env vars (`HOME_STORAGE_WARN_TOTAL_MB`, `HOME_STORAGE_CACHE_SECONDS`), no secret, no bucket, no sidecar. ⚠ **It is built — read `PRD.md` §V9-12 (as built) before `HANDOFF-11`, which carries a correcting banner and is wrong in nine places.** Notably **D214 was DECLINED** (the app does not hold Litestream's credentials), `dbstat` **is** available, there are **four** FTS5 indexes and not three, `/ws` was **fixed** rather than named, and **five acceptance criteria are unmet** — the largest being that the trigger-listener redaction has no test.
>
> **Index status (2026-09-06): v6 through v10.2 are all BUILT AND LIVE.** Home runs **eleven** modules on the DEPLOYED **OpenAPI 0.14.0**. **v11 is built: all three PRs** and moves the repository's contract to **0.15.0**, which reaches the droplet on the next Coolify build of `main` — see the v11 entry below.
>
> ⚠ **This index went stale between v9 and v11 and the correction is recorded rather than quietly applied.** It described a five-module app in its own title, carried no v10 entry at all, and its build-guide table stopped at `HANDOFF-11`. v10 (Chat), v10.1, v10.2 and `HANDOFF-12-chat.md` were all missing. They are added below. ⚠ **Two shipped passes had no PRD or CHANGELOG entry anywhere** — `#46`'s same-day reminder lead (which moved the contract to **0.14.0 with no changelog entry**) and `#47`/`#48`'s status.tilcer.cz integration, whose only written record was the README. ✅ **Written 2026-09-06 as v11's first commit**: `PRD.md` **§V10-16** and **§V10-17**, plus one CHANGELOG entry between v10.1 and v10.2 — and the v10.2 entry itself, which this bundle's CHANGELOG copy had lost. ⚠ The **Nextcloud masters are still to be re-synced by hand**; nothing in this repo can do that.
>
> **v11 (spec 2026-09-06 — ALL THREE PRs BUILT; §V11-12 is the as-built record):** the second version of Home that adds **no module** — and the first that adds a **credential**. An **MCP server inside the same binary** at `POST /mcp`, mounted OUTSIDE the `/api` group and authenticated by a personal access token, publishing **39 tools** through a **fifth registered catalog** (`platform/mcp`). Narrow on verbs, wide on reads: nothing destructive is exposed **at all** (absent, not gated), and a token sees everything its owner sees — private roots and chat bodies included — so the review effort belongs on **§V11-4 FR-M11's seventeen-row leak table**. Build guide **`HANDOFF-13-mcp.md`**, scope brief `V11-MCP-brief.md`, design brief `HANDOFF-design.md` **§v11**, `PRD.md` **§V11-1…§V11-11** + `openapi.yaml` **0.15.0**, decisions **D275–D323**. ⚠ Shipped as **THREE PRs** (D323) and the order was not negotiable — PR 1 carried the whole risk. ⚠ The MCP surface itself is **not** in `openapi.yaml` and cannot be: its contract is `backend/mcp-manifest.json`, **generated from the live registry by `internal/platform/mcp`'s own golden test** — which is where it has to live, because that package's harness composes every real provider over a real database and a manifest built from anything less describes a fixture. `internal/arch/mcp_completeness_test.go` is the OTHER half: it reads the committed file and checks it against the module directories and the `McpModule` enum without needing a database at all (D320). ⚠ The deploy still needs the `/mcp` Coolify path route, which nothing in the repo can do (D318).
>
> **v10 (spec 2026-08-26 — BUILT AND DEPLOYED 2026-08-28; v10.1 2026-08-30; v10.2 2026-09-05/06):** adds the **eleventh** module, **Chat** — and with it a **third access axis**. v1–v8 were household-visible, v9 added *ownership*, v10 adds **membership**: a conversation is readable by the people in it, from their `effective_from` onward. Build guide `HANDOFF-12-chat.md`, scope brief `V10-chat-brief.md`, `PRD.md` §V10-1…§V10-11 + as-built **§V10-12/13/14/15**, `openapi.yaml` 0.12.0 → **0.13.0**, decisions **D216–D274**. ⚠ `platform/ws` grew a per-user identity and `platform/storage` a `GroupSource`; **chat messages write no audit event** (D231/D256), the only primary mutation in Home that does not.  Build guides: v7 = `HANDOFF-9-garden.md` (Zahrada, block 10), v8 = `HANDOFF-10-electricity.md` (Elektřina, block 11). As-built reconciliations live in `PRD.md` **§V7-12** and **§V8-12** — read them before trusting any version-scoped section below. ⚠ **One process lesson from v7 + v8: `backend/openapi.yaml` was never updated by either**, so the served contract sat three versions behind until a separate pass fixed it. **v9 did not repeat it** — 0.11.0 shipped in the same PR. Keep it on the checklist. ⚠ **v9's own lesson is the mirror image:** the code was current and the *Nextcloud docs* were not, for four days. The paragraph below is kept as written at v6 spec time.
>
...
| `HANDOFF-9-garden.md` *(v7)* | `garden` — Zahrada (crop knowledge base, beds, season plan with on-read checks, work calendar, harvest, storage; `garden.prace` widget) | foundation, spine, **`platform/scheduler`** |
| `HANDOFF-10-electricity.md` *(v8)* | `electricity` — Elektřina (VT/NT readings, versioned ceník, zálohy, on-read nedoplatek/přeplatek). **No widget, metric, list or push** | foundation, spine |
| `HANDOFF-11-privacy-storage.md` *(v9)* | **Not a module.** `notes` + `documents` gain a private second root; `admin` gains Úložiště + the purge screen; **`platform/storage`** is the fourth registered catalog | foundation, spine, `notes`, `documents`, `admin`, **`platform/{audit,blobstore,storage}`** |
| `HANDOFF-12-chat.md` *(v10)* | `chat` — Chat (conversations, membership floor, attachments, koš, clean-up, the move to Dokumenty). **`platform/ws` grows an identity; `platform/storage` grows a `GroupSource`** | foundation, spine, `platform/ws`, `platform/storage`, `admin` |
| `HANDOFF-13-mcp.md` *(v11)* | **Not a module, and not a migration block either.** A second **front door** to all eleven: `platform/mcp` as the fifth registered catalog, a personal-access-token credential, 39 tools, three resource templates, five prompts. Every module gains a provider; none gains a table | foundation, spine, `platform/auth`, `logging`, and every feature module |
