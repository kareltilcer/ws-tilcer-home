...
⚠ **As built it reads `v1.10.2 · a3f9c2e` — and the artboards do not say `v10.2` either (D274).** The bundle draws this label twice, and both times it reads **`v10.1 · a3f9c2e`**: the two `appVersion` fields in `design/v10_2/Home.dc.html`, and hard problem 57 in the same file. There is no `v10.2 ·` anywhere in the bundle. That is not drift on the design's part — the bundle was synced against a repo that was still at v10.1 (`github.md`, *Last sync*), and the version label is **new in this bundle**, so its sample number was never this release's. The line above quotes the release this pass shipped, not the string the artboards carry. The version half is read out of `frontend/package.json` so the manifest and the label cannot say two different things, and npm demands three-part semver — so the release lives in `minor.patch` and the leading `1` is a constant that means nothing. Recorded as a difference rather than as drift, and it joins §V10-15's *What was not followed from the bundle* list, which now runs to four: what the bundle specifies is the *shape* of the label — `verze · krátký commit`, mono and quiet — and the shape is unchanged.

⚠ **One token was not followed, and it was measured rather than argued.** The artboards specify `--subtle` for the version label. `--subtle` on `--s1` is **4.29:1** in the light theme, under the 4.5:1 AA bar for text this size — the identical measurement `theme/globals.css` already recorded for the foreign bubble's author label, where it chose `--muted` (6.81:1). The same swap fixed **five** usages: three the axe sweep named — one `admin` tag in the sidebar, two on Nastavení, and the sidebar's had been failing the suite since before this pass — and two the sweep can never see, the `admin` tag inside the mobile "Více" sheet and a line that renders only after somebody closes the browser's permission dialog without deciding. ⚠ **And `--s1` is not the worst ground `--subtle` is drawn on.** In the sheet it sits on `--s2` at 4.04:1 and, inside the active row, on `--accent-soft` at **3.48:1** — a different and worse pairing than the one this note started from. The design system's own record shows the audit lifting the DARK `--subtle` 0.560 → 0.660 while the light value stayed at 0.580; that is where the general fix belongs, and it needs the sheet's two grounds in front of it, not only `--s1`.
...
### As built (2026-09-07) — what was followed, and the three places it was not

The bundle was followed. Both screens, the mint dialog's three fields, the four expiry choices, the reveal's refusals, the six-column list with its four text-distinguished states, the third level-1 group, the neutral Log chip, and the Odvolat confirmation that names the token without making anybody type it. Three differences, each recorded rather than quietly taken.

**1. The `nepoužito` marker is drawn once, not twice.** `mcpVals`'s `showMark` is `state !== 'active'`, which puts *nepoužito* in the name column's marker **and** in the *naposledy použito* column, where the artboards also render it in italics. Two identical words in one row cost a line at 375 px and say nothing the second time, so the marker is emitted for `soon`, `expired` and `revoked` only, and the never-used state lives in its own column. The five states are all still distinguished by text — which is the rule the marker exists to satisfy — and `markLabel` asserts the absence rather than leaving it to a reader.

**2. The reveal's snippet container is focusable, which no artboard shows.** ⚠ **The axe sweep is what found it, on the one screen where it matters most.** A scrollable region with no focusable content cannot be scrolled from the keyboard at all (WCAG 2.1.1) — so a keyboard-only member would have seen the first two thirds of a secret they will never be shown again, with no way to reach the rest. The container now carries `tabIndex={0}` and a label; nothing about the drawing changes.

**3. `--accent` on `--accent-soft` was fixed as well as `--subtle`.** The bundle's tokens section commits to *žádná nová barva · jedna opravená hodnota* and ships `--subtle` (light 0.580 → 0.515, dark 0.660 → 0.700), which is exactly what was built. But adding `/administrace` to the axe sweep — the strip this addendum sends an eighth tab to — measured **3.77:1** in the dark theme and **4.33:1** in the light one for the selected tab's label, at 12.5 px. The pairing is used by eight controls including the side nav, so it is fixed in the **token** (soft 0.360 → 0.300 dark, 0.930 → 0.950 light; 4.74 and 4.57 now) rather than at eight call sites. That keeps every drawing unchanged and honours §v10.2's own conclusion, quoted in `NastaveniPage.tsx`: *the pairing itself is wider than this screen and belongs to the design bundle's tokens, not to a per-usage fix here.*

⚠ **And a fourth measurement was NOT acted on.** `--warn` on `--s1` is **4.07:1** in the light theme at 12 px, on Administrace → Rozeslat. It is a serious AA failure on a screen this addendum does not touch, so it is recorded here and left alone; `/administrace` is out of the page-level axe path list with the measurement written above it, and the Asistenti tab carries its own sweep. **It belongs in the next bundle's tokens section**, the way `--subtle` belonged in this one.

**Definition of done, against what was built.** The five list states, the three reveal states and the two Log rows are covered by unit tests over the pure derivations plus a Playwright run that mints a real token against a real backend, proves the reveal survives Escape and an overlay click, runs axe over the open modal, closes it with *Hotovo*, revokes, and reads both audit rows and the three origin chips back out of the Log. ⚠ **What is NOT covered is a second member's session** — the frontend e2e runs as one member under the dev auth bypass, so the owner column on the Administrace tab is exercised with one owner. Every access rule behind it is asserted from the attacker's side in `internal/platform/mcp`, against a real database, with a second member who really is a different person.
