...
- **Security:** no new surface. Reads member-gated, writes `editor`/`admin` + CSRF, hard delete admin-only. Money values are integers — no float ever reaches the database, and the only float in the system is inside `math.Round` in the formula.
...
- **D84 — Finance is an ordinary module, open to all roles.** Reads for every member including `reader`; writes `editor`/`admin`; hard delete `admin`. **Not** admin-gated, unlike `admin`/`logging`. It sits in the **"více" overflow for everyone**, beside Dokumenty — a once-a-month destination does not earn one of the four thumb tabs.
