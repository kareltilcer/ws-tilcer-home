...
- **`events` (household — shared completion, D68):** `events.pripominky_today` = the **current** reminders: the `events.pripominky` widget's non-overdue rows — lead open (`occurrence − reminder_lead ≤ today`), day not yet passed — resolved through the widget's own selection, completed or not (**D99**). `events.pripominky_today_open` = of those, only the rows without a completion (the widget's non-overdue rows verbatim). `events.overdue_open` = past occurrences within the lookback still uncompleted. `events.due_within_7d` = occurrences due in `[today, today+7d]` — the one key anchored to event dates rather than reminder dates, because a look-ahead is a question about the calendar. All of them use the same bounded RRULE expansion the widget does. **List-only (no metric twin):** `events.pripominky_active` = every active reminder incl. overdue, i.e. the `events.pripominky` widget verbatim, resolved through the widget's own selection (**D100**).
