...
8. **A nudge that informs without nagging.** *"poslední odečet před 47 dny"* is the entire retention mechanism the user permitted. Get the register right, and decide explicitly whether it escalates.
...
- [ ] **The nudge line** at 4 / 47 / 200 days, with the escalation decision stated.
...
*Backend + frontend for `electricity` (`HANDOFF-10-electricity.md`) can be built from `PRD.md` §V8 + `openapi.yaml` 0.10.0; the **four Elektřina screens and the "Více" nav update** are reconciled against this addendum once approved — the same rule v2–v7 used. The three non-registry host maps (`AppShell` OVERFLOW, the Log browser's `MODULES`, `admin/listener.go`'s `inAppURL`), the `compute.go` purity test and the no-widget assertion are build concerns, not design.*
