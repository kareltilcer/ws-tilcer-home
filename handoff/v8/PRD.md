...
> Status: **v8 — LIVE** (Elektřina / `electricity`, built and deployed 2026-08-21) · **v7 — LIVE** (Zahrada / `garden`, built and deployed 2026-08-21) · **`fin` is not yet retired** — the migration is **verified** (§V6-12 step 4 done) and only the ordered switch-off (step 5) is outstanding. v8 added a tenth module, **Elektřina** (`electricity`) — VT/NT readings, a date-versioned ceník and zálohy answering *vyjdou zálohy, nebo doplatím?*. v7 added a ninth, **Zahrada** (`garden`). v6 added an eighth, **Finance** (`finance`) — absorbing the standalone `fin` service (`fin.tilcer.cz`) and migrating its data. v5 added the **Administrace** (`admin`) module (Web Push notifications) and an installable, reads-only-offline **PWA** on top of v4 (Dokumenty). Self-hosted login (Mode B), widget dashboard and modular architecture unchanged throughout. Decisions **D1–D175** (§10 for D1–D50; the **v5 section** for D51–D74; **§V5-12 for D75–D80 — shipped after the v5 spec froze**; the **v6 section** for D81–D98, with **D99/D100 taken during the v6 build**; **§V6-13** = the v6 as-built reconciliation; the **v7 section** for D101–D132, with **§V7-12 = the v7 as-built reconciliation, D163–D168**; the **v8 section** for D133–D162, of which **D157–D160 were forced back into the spec by the `HANDOFF-10` pass** and **D161–D162 by the implementation-planning pass**, with **§V8-12 = the v8 as-built reconciliation, D169–D175**); v-deltas in `CHANGELOG.md`. **Ten modules live.** · Owner: Karel · Last updated: 2026-08-21
> Companion spec: `openapi.yaml` (OpenAPI 3.1 — **0.10.1 = v8 AS BUILT, and is what is deployed**; 0.10.0 was the v8 spec, 0.9.0 the v7 spec, 0.8.0 the v6 build) · Notes: `notes.md` · Design brief: `HANDOFF-design.md` (v2–**v8** addenda; the v6 palette question is **RESOLVED — Path A**, the v8 nudge-escalation question is **RESOLVED by the build — D175**) · Build: `HANDOFF-1…10-*.md` (v5 = `HANDOFF-7-admin.md`, v6 = `HANDOFF-8-finance.md`, v7 = `HANDOFF-9-garden.md`, v8 = `HANDOFF-10-electricity.md` — all built)
