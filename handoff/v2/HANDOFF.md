# Home — Implementation Handoff (Claude Code) — v2

> **Read first:** root `CLAUDE.md` (project conventions), then `PRD.md` (v2 — the source of truth), `openapi.yaml` (v0.3.0), and `CHANGELOG.md` (what changed from v1). `notes.md` holds decisions D1–D29 in short form; `HANDOFF-design.md` is the design brief (v2 addendum for login + widget host still pending — see §8).
>
...
Build the foundation, then 1 → 2 → 3 → 4. The spine is first (every mutation writes through it in the same transaction); the dashboard host is last (it renders the others' widgets).
...
ws-tilcer-home/
  ...
  frontend/
    src/
      ...
      modules/
        todo/  events/  logging/  dashboard/   # each: pages + widgets + query hooks
