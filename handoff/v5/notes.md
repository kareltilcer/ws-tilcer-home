...
- **Attachments → text + external links only** (Q2). No uploads, no embedded files, **no blob storage added**. → D34.
...
- **D34** **text + external links only** — no uploads/embedded attachments, **no blob storage** (Markdown may reference external image URLs)
