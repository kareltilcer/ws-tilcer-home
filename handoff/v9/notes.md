...
## v9 — Soukromé položky a Úložiště (spec 2026-08-21, D176–D215, NOT BUILT)
...
**One item left, and it belongs to the build:** whether `modernc.org/sqlite` exposes `dbstat`. Very likely yes — `modernc.org/libsqlite3` publishes `DBSTAT_PAGE_PADDING_BYTES`, which sits inside the `#if defined(SQLITE_ENABLE_DBSTAT_VTAB)` guard in the amalgamation and is only transpiled when the flag is set. Inference from a package index, so the boot probe stays and the fallback must be **exercised**.
