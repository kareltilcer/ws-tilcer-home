...
> Status: **v6 — LIVE** (deployed 2026-08-18) · **v7 — SPEC** (Zahrada / `garden`, frozen 2026-08-18, not yet built) · **`fin` is not yet retired** — the live verification and switch-off (§V6-12 steps 4–5) are the only outstanding work. v6 added an eighth module, **Finance** (`finance`) — absorbing the standalone `fin` service (`fin.tilcer.cz`), migrating its data and **retiring it**. v5 added the **Administrace** (`admin`) module (Web Push notifications) and an installable, reads-only-offline **PWA** on top of v4 (Dokumenty). Self-hosted login (Mode B), widget dashboard and modular architecture unchanged throughout. Decisions **D1–D132** (§10 for D1–D50; the **v5 section** for D51–D74; **§V5-12 for D75–D80 — shipped after the v5 spec froze**; the **v6 section** for D81–D98, with **D99/D100 taken during the v6 build**; **§V6-13** = the v6 as-built reconciliation; the **v7 section** for D101–D132); v-deltas in `CHANGELOG.md`. **Eight modules live; a ninth (`garden`) is specified.** · Owner: Karel · Last updated: 2026-08-18
> Companion spec: `openapi.yaml` (OpenAPI 3.1 — **0.8.0 = the deployed build**, **0.9.0 = the v7 spec**) · Notes: `notes.md` · Design brief: `HANDOFF-design.md` (v2–**v6** addenda; the v6 palette question is **RESOLVED — Path A**) · Build: `HANDOFF-1…9-*.md` (v5 = `HANDOFF-7-admin.md`, v6 = `HANDOFF-8-finance.md`, v7 = `HANDOFF-9-garden.md` — pending)
