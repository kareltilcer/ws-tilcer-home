...
- [ ] **The gestures have never been touched by a finger.** The state machine is unit-tested against synthetic pointer events (fourteen cases, including the ones that matter: a scroll flick is not a tap, a long press's own release is not a tap, a committed swipe does not pair into a double tap) — but a real thumb on a real phone is what decides whether 500 ms and 64 px are the right numbers, and no test can answer that.
