...
- [ ] **The label has been seen with a real commit; the COOLIFY half of the path has not.** Under D274 it was run with `VITE_APP_COMMIT` set to a real sha and rendered `v1.10.2 · b4a3b32` in both places — side nav foot and *Více* sheet — so `import.meta.env` → `shortCommit` → label is observed rather than reasoned about, and a production `npm run build` with the arg set carries the sha into `dist/assets/*.js`. ⚠ **The outer link has now been observed, and it failed.** The first deploy with *Include Source Commit in Build* on and the cache cleared still baked an empty `VITE_APP_COMMIT` into `dist/assets/*.js` — the deployed bundle was the right code (`1.10.2` present) with an empty commit. Coolify does **not** pass `SOURCE_COMMIT` as a `--build-arg` the way it passes the `VITE_*` variables you define yourself: for the Dockerfile build pack it **splices `ARG SOURCE_COMMIT=<sha>` into the file at line index 1**, assuming line 0 is a `FROM`. Line 0 of `frontend/Dockerfile` is `# syntax=docker/dockerfile:1`, so the injected line lands **before the first `FROM`** — Docker *global* scope, invisible inside a build stage unless the stage re-declares the name **without** a default. `ARG SOURCE_COMMIT=""` is a re-declaration *with* one, so it shadowed the injected sha with an empty string. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing. Fixed, and reproduced first: the real Dockerfile was built twice with Coolify's splice applied, `=""` against bare, `[]` against the sha. ⚠ **What is STILL unobserved is the fixed path on the real platform** — everything here is a local reproduction of Coolify's documented splice, not a Coolify build. If a rebuild still shows no commit, the next thing to read is the build log for an injected `ARG SOURCE_COMMIT=` line: absent, Coolify is not injecting at all and the problem is upstream of this repo. ⚠ **The cache cost of turning it on was overstated** (D274): `npm ci` sits above `COPY . .` where the arg cannot reach it, so only a commit touching nothing under `frontend/` pays anything at all. (`npm ci` *does* re-run on a RELEASE commit, because D274 put the version in `package.json` — that is D274's own cost, and the toggle adds nothing to it.)
