...
- [ ] **The label has been seen with a real commit; the COOLIFY half of the path has not.** Under D274 it was run with `VITE_APP_COMMIT` set to a real sha and rendered `v1.10.2 · b4a3b32` in both places — side nav foot and *Více* sheet — so `import.meta.env` → `shortCommit` → label is observed rather than reasoned about, and a production `npm run build` with the arg set carries the sha into `dist/assets/*.js`. ⚠ **The outer link has now been observed, and it failed.** The first deploy with *Include Source Commit in Build* on and the cache cleared still baked an empty `VITE_APP_COMMIT` into `dist/assets/*.js` — the deployed bundle was the right code (`1.10.2` present) with an empty commit. The cause is one sentence: **a default written in the Dockerfile beats the `ARG` line Coolify splices in**, and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, asserted by `version.test.ts` along with its position. ⚠ **Coolify supplies build variables two ways at once, and the two are NOT equally strong** — the inequality is the bug. (1) A **key-only `--build-arg 'KEY'`**: `generateDockerBuildArgs()` emits the key alone, and Docker reads the value from the environment of the build process, which Coolify fills by sourcing `/artifacts/build-time.env`. When the key is in that file this beats every default in the Dockerfile, `=""` included; when it is not, the flag carries nothing at all — measured, indistinguishable from passing nothing — and the file's own default decides. (2) A **spliced `ARG KEY=<value>` line** after *every* `FROM` (older versions: a hardcoded line index 1, falling *before* the first `FROM` when line 0 is a comment as it is here — [coollabsio/coolify#7118](https://github.com/coollabsio/coolify/issues/7118)). That is the weak path, and an in-file default beats it from either side: above the `FROM` the stage's `=""` shadows the global, in-stage it resets the injected value. Reproduced on the real Dockerfile across all four supply paths — global splice, in-stage splice, `--build-arg`, nothing at all — reading the value back out of `dist/assets/*.js`; bare is the only spelling correct in all four. ⚠ **`SOURCE_COMMIT` and the user-defined `VITE_*` variables are NOT delivered the same way, and that asymmetry is what broke.** (An intermediate reading of this failure denied the asymmetry outright; that was wrong in the other direction, and it made the failure unexplainable — a `--build-arg` that fires beats `ARG SOURCE_COMMIT=""`, measured, so if one had fired for `SOURCE_COMMIT` the label would have been correct.) `SOURCE_COMMIT` does go into the same `env_args` collection (`generate_env_variables()`) and does get a `--build-arg` — but a key-only flag is only as good as the environment behind it. Build-time variables you define are written into `build-time.env` unconditionally, so path 1 always fires for them and their `=""` never gets the chance to lose; `SOURCE_COMMIT` reaches that file only through `generate_coolify_env_variables(forBuildTime: true)`, gated on the toggle **and skipped outright when the application defines its own variable of that name** — while the splice carries it with no such guard. So it can arrive by path 2 alone, which `ARG SOURCE_COMMIT=""` beats. ⚠ **What is STILL unobserved is the fixed path on the real platform** — everything here is a local reproduction, not a Coolify build. If a rebuild still shows no commit, check in this order: an application variable named `SOURCE_COMMIT` or `VITE_APP_COMMIT` (the one failure the fixed Dockerfile cannot survive — an empty build-time row fires path 1 with an empty value and beats bare `ARG` and the splice alike); then, under *Show Debug Logs*, whether `SOURCE_COMMIT` appears in the `cat` of `/artifacts/build-time.env` (path 1 live) or only as an `ARG SOURCE_COMMIT=` line in the echoed final Dockerfile (path 2 live); then the toggle and *Build arguments*, where ARG injection can be switched off wholesale. The bundle is what settles it. ⚠ **The cache cost of turning it on was overstated** (D274): `npm ci` sits above `COPY . .` where the arg cannot reach it, so only a commit touching nothing under `frontend/` pays anything at all. (`npm ci` *does* re-run on a RELEASE commit, because D274 put the version in `package.json` — that is D274's own cost, and the toggle adds nothing to it.)
