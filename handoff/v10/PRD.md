...
Four usages swapped to `--muted` (6.81:1 light, 7.88:1 dark), which is the swap `theme/globals.css` already made once, for the foreign bubble's author label, with the same measurement written beside it. ⚠ **The pairing is wider than four usages**: `--subtle` on `--s1` fails AA in the light theme *everywhere it appears*, and the design system's own note records the audit that lifted the DARK value 0.560 → 0.660 while the light value stayed at 0.580. That is a token decision for the design bundle, not something an implementation PR gets to change under nine other screens — recorded, not fixed.
...
- [ ] **`--subtle` on `--s1` fails AA in the light theme app-wide** (4.29:1). Four usages are fixed here because they were on the two screens the axe sweep covers; the token pairing itself needs a design decision, and every screen outside `/`, `/okno` and `/nastaveni` is unmeasured.
