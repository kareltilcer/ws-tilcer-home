...
- **The label reads `v1.10.2`, not the artboards' `v10.2`** (D274). npm demands three-part semver, so the release is carried in `minor.patch` and the leading `1` is a constant that means nothing; the alternative was reconstructing the changelog's spelling from the manifest, which makes the string in a bug report ungreppable in the file a release bumps. What the artboards specify is the label's *shape*, and the shape is unchanged.
