...
- [ ] **The COOLIFY half of the path has still not been seen WORKING.** Under D274 the label was run with `VITE_APP_COMMIT` set to a real sha and rendered `v1.10.2 · b4a3b32` in both places — side nav foot and *Více* sheet — so `import.meta.env` → `shortCommit` → label is observed rather than reasoned about, and a production `npm run build` with the arg set carries the sha into `dist/assets/*.js`. ⚠ **The outer link has now been exercised once, and it failed.** The first deploy with *Include Source Commit in Build* on and the cache cleared still baked an empty `VITE_APP_COMMIT` into `dist/assets/*.js` — the deployed bundle was the right code (`1.10.2` present) with an empty commit. The defect is one sentence: **a default written in the Dockerfile beats the `ARG SOURCE_COMMIT=<sha>` line Coolify splices in** — in-stage it resets the injected value, above the `FROM` it shadows it — and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, asserted by `version.test.ts` along with its position. Reproduced on the real Dockerfile across all four supply paths — global splice, in-stage splice, `--build-arg`, nothing at all — reading the value back out of `dist/assets/*.js`; bare is the only spelling correct in all four. ⚠ **WHY THE DEPLOY FELL THROUGH TO THE SPLICE IS NOT ESTABLISHED, and two readings have already been wrong about it.** The first denied that Coolify treats `SOURCE_COMMIT` differently from a user build variable; the second asserted the difference *as the cause*. What holds up: Coolify passes `SOURCE_COMMIT` as a key-only `--build-arg` whose value it sources from `/artifacts/build-time.env`, and a flag that carries anything beats every default in the Dockerfile, `=""` included (measured) — so if it had fired, the label would have been correct, and something kept `SOURCE_COMMIT` out of that file. The one known way that happens is an application variable of the same name: `generate_coolify_env_variables(forBuildTime: true)` skips Coolify's own value when the app defines that key, while the splice carries it unguarded. Whether that was the case here was never checked before the fix shipped. **The bare `ARG` is correct under every path either way**, which is why it shipped without waiting for the answer. ⚠ **What is STILL unobserved is the fixed path on the real platform** — everything here is a local reproduction, not a Coolify build. If a rebuild still shows no commit, check in this order: an application variable named `SOURCE_COMMIT` or `VITE_APP_COMMIT` (the one failure the fixed Dockerfile cannot survive — an empty build-time row fires path 1 with an empty value and beats bare `ARG` and the splice alike); then, under *Show Debug Logs*, the echoed final Dockerfile, where an `ARG SOURCE_COMMIT=<sha>` line under each `FROM` is the splice firing with its value — path 1 is only half visible there, because the echoed build script shows the key-only `--build-arg` and never what it carries, and the `cat` of `/artifacts/build-time.env` that would is gated on Coolify's own dev mode rather than on that toggle; then the toggle and *Inject Build Args to Dockerfile*, where ARG injection can be switched off wholesale. ⚠ **Both controls are named here as they were on the instance that produced this bug, which is the pre-redesign panel** — Coolify's *Shadow UI redesign* of 2026-08-03 turned them into a *Source commit availability* dropdown (⇒ *Available during build*) and a *Build arguments* dropdown (⇒ *Inject build args automatically*), so on a newer instance look for those rather than concluding the controls are gone. The bundle is what settles it. ⚠ **The cache cost of turning it on was overstated** (D274): `npm ci` sits above `COPY . .` where the arg cannot reach it, so only a commit touching nothing under `frontend/` pays anything at all. (`npm ci` *does* re-run on a RELEASE commit, because D274 put the version in `package.json` — that is D274's own cost, and the toggle adds nothing to it.)
