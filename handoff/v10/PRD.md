...
- [ ] **The label has been seen with a real commit; the COOLIFY half of the path has not.** Under D274 it was run with `VITE_APP_COMMIT` set to a real sha and rendered `v1.10.2 · b4a3b32` in both places — side nav foot and *Více* sheet — so `import.meta.env` → `shortCommit` → label is observed rather than reasoned about, and a production `npm run build` with the arg set carries the sha into `dist/assets/*.js`. ⚠ **The outer link has now been observed, and it failed.** The first deploy with *Include Source Commit in Build* on and the cache cleared still baked an empty `VITE_APP_COMMIT` into `dist/assets/*.js` — the deployed bundle was the right code (`1.10.2` present) with an empty commit. The cause is one sentence: **a default written in the Dockerfile beats the value Coolify injects**, and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, asserted by `version.test.ts`. ⚠ **The fix deliberately does not depend on which path the value takes.** Coolify supplies build variables two ways at once — a key-only `--build-arg` whose value it exports into the build environment, and `ARG KEY=<value>` lines spliced into the Dockerfile — and the splice position has moved between versions (a hardcoded line index 1, which falls *before* the first `FROM` when line 0 is a comment as it is here — [coollabsio/coolify#7118](https://github.com/coollabsio/coolify/issues/7118) — versus after *every* `FROM` in current builds). Both positions break against an in-file default from opposite sides: before the `FROM` the stage's `=""` shadows the global, after it the `=""` resets the injected value. Reproduced on the real Dockerfile across all four supply paths — global splice, in-stage splice, `--build-arg`, nothing at all — reading the value back out of `dist/assets/*.js`; bare is the only spelling correct in all four. ⚠ **`SOURCE_COMMIT` is not special to Coolify**, so the earlier reading of this failure as an asymmetry between it and the user-defined `VITE_*` variables was wrong: they ride the same two paths, and the `=""` those args carry is not a structural guarantee either — they survive because the `--build-arg` also fires. ⚠ **What is STILL unobserved is the fixed path on the real platform** — everything here is a local reproduction, not a Coolify build. If a rebuild still shows no commit, read the echoed final Dockerfile under *Show Debug Logs* for an `ARG SOURCE_COMMIT=` line under each `FROM`; absence does not by itself convict the platform, because the value can still arrive as a key-only `--build-arg` and ARG injection can be switched off in the application's advanced settings. The bundle is what settles it. ⚠ **The cache cost of turning it on was overstated** (D274): `npm ci` sits above `COPY . .` where the arg cannot reach it, so only a commit touching nothing under `frontend/` pays anything at all. (`npm ci` *does* re-run on a RELEASE commit, because D274 put the version in `package.json` — that is D274's own cost, and the toggle adds nothing to it.)
