...
⚠ **As built it reads `v1.10.2 · a3f9c2e`, not the artboards' `v10.2` (D274).** The version half is read out of `frontend/package.json` so the manifest and the label cannot say two different things, and npm demands three-part semver — so the release lives in `minor.patch` and the leading `1` is a constant that means nothing. The artboard string is the design's, and this is the fourth recorded difference from the bundle rather than drift: what is drawn is the *shape* of the label, and the shape is unchanged.
