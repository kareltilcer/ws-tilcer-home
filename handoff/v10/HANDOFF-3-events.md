...
> **Scope:** all-day future events with optional recurrence and one optional in-app reminder — **plus two dashboard widgets, `events.pripominky` and `events.tento-mesic`** (§8 below).
> **v2:** packaged as a module per `HANDOFF.md` §3 (own routes/migrations/audit/widgets, registered via the core; no cross-module imports). Auth is Mode B — authorization is from the home **session**; the role gate (writes = `editor`/`admin`) is unchanged. The v1 dashboard reminder list is now this module's `events.pripominky` **widget provider** (§8); `events.tento-mesic` is a new look-ahead widget. Event/recurrence behaviour below is unchanged from v1.
...
- [ ] Both widget providers registered (§8).

## 8. Widget providers (FR-E7, FR-E8) — v2

This module contributes two dashboard widgets through the `WidgetProvider` contract (`HANDOFF.md` §3). The host calls them; they never expose this module's tables.

### `events.pripominky` — Připomínky (FR-E7)
- Title **"Připomínky"**, default size **narrow**, not admin-only.
- **`Data(ctx, user)`**: the active reminders — for each non-archived reminder-enabled event, the **earliest uncompleted occurrence** within `HOME_DASHBOARD_LOOKBACK_DAYS`, included when `today >= occurrence − lead`; **at most one per event**; overdue flagged and sorted first, then by date. Shape = `DashboardReminder`. This is the v1 dashboard reminder list, now a provider — the "one per event / lookback bound / activation boundary" rules from the v1 dashboard tests move here.
- **Frontend component:** overdue rows distinct and first (restrained accent, not a wall of red); **2000 ms press-and-hold done** (D22, immediate keyboard path) → `POST /api/events/{id}/complete` with `meta.via="dashboard"`; row tap opens event detail.

### `events.tento-mesic` — Tento měsíc (FR-E8)
- Title **"Tento měsíc"**, default size **narrow**, not admin-only. **Read-only** — no done control.
- **`Data(ctx, user)`**: upcoming occurrences from today through end of the current month (or next `N` days), sorted by date. Shape = `Occurrence` list. This is the look-ahead that relaxes v1 D16 (§10 D16-adjusted).
- **Frontend component:** a compact upcoming list; tap opens event detail; no completion.

**Tests:** `events.pripominky` matches the v1 reminder rules (one per event, activation boundary in `Europe/Prague`, advance-on-complete, lookback drop-off, overdue ordering); `events.tento-mesic` returns only future-through-month occurrences and exposes no completion path; both compute via the provider without the host touching event tables.
