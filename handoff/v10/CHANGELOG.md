...
⚠ **Coolify excludes `SOURCE_COMMIT` from the build by default** — *Include Source Commit in Build* is the toggle, and without it the label reads `v1.10.2` alone, which still names the release. **The cache reason for leaving it off does not apply to `frontend/Dockerfile`**: the layer under the arg is `RUN npm run build`, while `RUN npm ci` sits above `COPY . .` and is keyed only on the two manifest files, so the install is never re-run either way. The only builds that pay are commits touching nothing under `frontend/` — on any commit that touches something, `COPY . .` has already invalidated the build layer.
