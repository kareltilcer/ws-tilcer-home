...
⚠ **AND THE TOGGLE ALONE WAS NOT ENOUGH.** The first deploy with *Include Source Commit in Build* on, cache cleared, still shipped a commit-less label — `VITE_APP_COMMIT` baked as `""` in `dist/assets/*.js` while every other `VITE_*` value arrived intact. The defect is one sentence: **a default written in the Dockerfile beats the `ARG SOURCE_COMMIT=<sha>` line Coolify splices in** — in-stage it resets the injected value, above the `FROM` it shadows it — and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, so `version.test.ts` asserts it, and its position, rather than leaving either to the comment above it. All four supply paths were built against the real Dockerfile and read back out of `dist/assets/*.js` — global splice, in-stage splice, `--build-arg`, and nothing at all — and bare is the only spelling correct in all four. ⚠ **What is NOT established is why the deploy fell through to the splice.** Coolify also passes `SOURCE_COMMIT` as a key-only `--build-arg` whose value it sources from `/artifacts/build-time.env`, and that flag, when it carries anything, beats every default in the Dockerfile — so had it fired, `=""` would have lost and the label would have been right. The one known way `SOURCE_COMMIT` misses that file is an application variable of the same name, which makes Coolify skip its own value while the splice carries it unguarded. The bare `ARG` is correct either way; the README carries the two paths, the source references and the order to check them in.
