...
⚠ **AND THE TOGGLE ALONE WAS NOT ENOUGH.** The first deploy with *Include Source Commit in Build* on, cache cleared, still shipped a commit-less label — `VITE_APP_COMMIT` baked as `""` in `dist/assets/*.js` while every other `VITE_*` value arrived intact. The cause is one sentence: **a default written in the Dockerfile beats the value Coolify injects**, and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, so `version.test.ts` asserts it rather than leaving it to the comment above it. ⚠ **The fix does not rest on knowing which path the value takes, and that is deliberate**: Coolify supplies build variables two ways at once — a key-only `--build-arg` whose value it exports into the build environment, *and* `ARG KEY=<value>` lines spliced into the Dockerfile — and where the spliced line lands has moved between versions (a hardcoded line index 1, which falls *before* the first `FROM` when line 0 is a comment as it is here — [coollabsio/coolify#7118](https://github.com/coollabsio/coolify/issues/7118) — versus after *every* `FROM` in current builds). Both positions break against an in-file default, from opposite sides: before the `FROM` the stage's `=""` shadows the global, after it the `=""` resets the injected value. All four supply paths were built against the real Dockerfile and read back out of `dist/assets/*.js` — global splice, in-stage splice, `--build-arg`, and nothing at all — and bare is the only spelling correct in all four. ⚠ **`SOURCE_COMMIT` is not special to Coolify**: it rides the same two paths as the variables you define yourself, so the `=""` the other `VITE_*` args still carry is not a structural guarantee either — they survive because the `--build-arg` also fires. They are left alone because they are not what broke.
