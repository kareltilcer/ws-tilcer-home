...
⚠ **AND THE TOGGLE ALONE WAS NOT ENOUGH.** The first deploy with *Include Source Commit in Build* on, cache cleared, still shipped a commit-less label — `VITE_APP_COMMIT` baked as `""` in `dist/assets/*.js` while every other `VITE_*` value arrived intact. Coolify does **not** pass `SOURCE_COMMIT` as a `--build-arg` the way it passes the `VITE_*` variables you define yourself: for the Dockerfile build pack it **splices `ARG SOURCE_COMMIT=<sha>` into the file at line index 1**, assuming line 0 is a `FROM`. Line 0 of `frontend/Dockerfile` is `# syntax=docker/dockerfile:1`, so the injected line lands **before the first `FROM`** — Docker *global* scope, invisible inside a build stage unless the stage re-declares the name **without** a default. `ARG SOURCE_COMMIT=""` is a re-declaration *with* one, so it shadowed the injected sha with an empty string. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing. The asymmetry is the tell: a real `--build-arg` beats an in-stage default, which is why the user-defined variables were fine and only the spliced one was empty. Reproduced by building the real Dockerfile twice with Coolify's splice applied — `=""` yields an empty `VITE_APP_COMMIT`, bare yields the sha.
