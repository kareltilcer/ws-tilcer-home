...
> **No API change** — OpenAPI stays at **0.14.0** and no migration ships. (0.13.0 was v10.1; `backend/openapi.yaml` has carried **0.14.0** since the same-day reminder lead `"0d"` was added to the event surfaces, which this file never got an entry for.) Decisions **D271–D274** (`PRD.md` §V10-10), as-built record in **§V10-15**. From the `design/v10_2` bundle plus three requests from Karel, all of them about the frame around the modules rather than about any module.
...
⚠ **AND THE TOGGLE ALONE WAS NOT ENOUGH.** The first deploy with *Include Source Commit in Build* on, cache cleared, still shipped a commit-less label — `VITE_APP_COMMIT` baked as `""` in `dist/assets/*.js` while every other `VITE_*` value arrived intact. The cause is one sentence: **a default written in the Dockerfile beats the `ARG` line Coolify splices in**, and the file said `ARG SOURCE_COMMIT=""`. It is now bare — `ARG SOURCE_COMMIT` — and that bareness is load-bearing, so `version.test.ts` asserts it, and its position, rather than leaving either to the comment above it. ⚠ **Coolify supplies build variables two ways at once, and the two are NOT equally strong** — which is the whole bug. (1) A **key-only `--build-arg 'KEY'`**: `generateDockerBuildArgs()` emits the key alone and Docker reads the value from the environment of the build process, which Coolify fills by sourcing `/artifacts/build-time.env`. In that file, this path beats every default in the Dockerfile, `=""` included; *not* in it, the flag carries nothing at all — measured, indistinguishable from passing nothing — and the Dockerfile's own default decides. (2) A **spliced `ARG KEY=<value>` line**, after *every* `FROM` (older versions: a hardcoded line index 1, which falls *before* the first `FROM` when line 0 is a comment as it is here — [coollabsio/coolify#7118](https://github.com/coollabsio/coolify/issues/7118)). That is the weak path, and an in-file default beats it from either side: above the `FROM` the stage's `=""` shadows the global, in-stage it resets the injected value. All four supply paths were built against the real Dockerfile and read back out of `dist/assets/*.js` — global splice, in-stage splice, `--build-arg`, and nothing at all — and bare is the only spelling correct in all four. ⚠ **`SOURCE_COMMIT` and the `VITE_*` variables are not delivered the same way, and that asymmetry is the reason one broke and the others did not.** It does go into the same `env_args` collection (`generate_env_variables()`), so it does get a `--build-arg` — but a key-only flag is only as good as the environment behind it. Build-time variables you define are written into `build-time.env` unconditionally, so path 1 always fires for them and their `=""` never gets the chance to lose; `SOURCE_COMMIT` reaches that file only through `generate_coolify_env_variables(forBuildTime: true)`, gated on the toggle **and skipped outright when the application defines its own variable of that name** — while the splice carries it with no such guard. So it can arrive by path 2 alone, which `ARG SOURCE_COMMIT=""` beats. The other args keep their `=""` because they are not what broke, not because they are safe by construction.
