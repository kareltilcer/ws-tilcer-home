...
// markArmed reports whether `type` applies where an empty caret sits — either armed for
// the next keystroke (storedMarks, which take precedence once anything has been toggled)
// or already carried by the text the caret is in. Both the code and the link branch below
// need exactly this question answered the same way.
...
export function applyFormat(ctx: Ctx, command: NoteFormatCommand) {
  ...
  switch (command) {
    ...
    case 'link': {
      ...
      // splices a SECOND link into the middle of the first one. Disarm the mark instead,
      // so what is typed next is plain text. (Crepe's own bar guards the same case.)
      if (state.selection.empty && markArmed(state, linkType)) {
