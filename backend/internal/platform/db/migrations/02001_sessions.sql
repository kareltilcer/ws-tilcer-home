-- platform core — Mode B session store (PRD §5, HANDOFF §F4, D29). Version block
-- 02000. Home owns its own session: the browser carries NO bearer token; each
-- request is authorized from this session, and roles are refreshed periodically
-- via auth /internal/token/mint.
--
