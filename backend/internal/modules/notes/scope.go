...
//
// Until v9 every row in `notes` was visible to every member, and roughly forty
// call sites were written against that. Privacy here is a SECOND ROOT rather than
// a per-item checkbox: a tree is addressed by the pair (visibility, owner_id), of
// which there are 1 + N for N members, and every read and write names one.
//
...
//
// This is deliberately duplicated in the `documents` module rather than shared
// with it: the v4 D40 precedent is that Dokumenty MIRRORS Poznámky's folder model
...
// ParseCreateScope parses the `scope` field of a CREATE body, and — unlike
// ParseScope — it distinguishes ABSENT from an explicit "shared".
//
...
// deliberate. Every by-id read enforces the audience in SQL, through viewerCond —
// GetNote, GetFolder, GetNoteImageForViewer — so a foreign private row never
// reaches Go and the handler's 404 needs no second check. A Go-side predicate
// beside them would be a SECOND way to spell the same rule, and the failure mode
// is the one v9 exists to prevent: a new route that loads unscoped, forgets to
// call it, and fails OPEN with nothing to notice.
//
// The refusal for a private item is still ALWAYS 404 and never 403 (D180) — that
// falls out of the store returning (nil, nil), which is byte-identical to an id
...
// SHARED note into their own private folder was being sent after a route that was
// never built. An error that prescribes an impossible remedy is worse than one that
// only refuses: it costs the reader the time it takes to discover the advice was
// wrong. Mirrors the documents twin.
...
// SQLite cannot add a table CHECK without rebuilding the table — and `notes`
// carries an explicit `seq INTEGER PRIMARY KEY` precisely because notes_fts is
